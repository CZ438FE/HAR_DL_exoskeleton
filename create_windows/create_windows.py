...
def add_args(group):
    ...
    group.add_argument(
        ...
        default=1000,
        type=int,
        help='Specify window length in ms, default: %(default)s'
    )
    group.add_argument(
        '-s', '--step_length',
        metavar='',
        ...
        default=500,
        type=str,
        help='Specify step length in ms, default: %(default)s'
    )
    group.add_argument(
        '-m', '--method',
        metavar='',
...
def create_windows(table: dict):
    ...
    labeled_files, error = file_utils.get_files(
        os.path.join(data_path, "labeled_data"), ".csv")
    if error:
        return

    # To keep the information which dataset was processed, it is stored
    table["seen_data_set"] = file_utils.detect_dataset(labeled_files)

    # When the results are being stored and the respective folders do not exist yet, create them at <dp>/windows/<saving_time>
    saving_time = datetime.now().strftime(
        '%Y-%m-%dT%H-%M') if output_date is None else output_date
    saving_folder = os.path.join(data_path, "windows", saving_time)
    ...
    # The following two objects are needed for further processing
    ...
    if not set(data_columns).issubset(set(needed_columns)):
        logger.error(
            f"The given data_columns are not a true subset of the columns of the needed_columns")
        return
    ...
    log_file, error = file_utils.initialize_log_file(os.path.join(
        saving_folder, "log.json"), os.path.join(data_path, "labeled_data", "log.json"), service, table)
    if error:
        logger.error(f"Initializing log file failed: {error}")
        return

    labeled_files, error = file_utils.reduce_files_to_handle(
        labeled_files, saving_folder, service)
    ...
    normalization_df, error = create_windows_utils.return_normalization_df(
        normalize, anonymize_file)
    if error:
        return

    # Each core needs all the params for the processing of the file
    list_of_file_dicts, error = create_windows_utils.create_list_of_file_dicts(
        labeled_files, needed_columns, window_length, step_length, filling_method, resampling_rate, data_columns, flatten, store_local, saving_folder, normalize, normalization_df)
    if error:
        return
    ...
    file_chunks = data_utils.split_files_to_handle_into_chunks(
        list_of_file_dicts, 24)
    if len(file_chunks) > os.cpu_count():
        logger.info(
            f"Smaller files are processed first, therefore a slowing of the processing of chunks is to be expected")
    ...
    for chunk_number, chunk in enumerate(file_chunks, start=1):
        ...
        with Pool() as pool:
            result = pool.map(
                create_windows_utils.create_windows_for_file, chunk)

        # track current progress / success of the processing
        log_file = file_utils.create_log_from_result_strings(
            result, os.path.join(saving_folder, "log.json"), service)
        ...
        if len(log_file[service]["occurred_errors"]) != 0:
            ...
            logger.warning(
                f"These errors occurred: {log_file[service]['occurred_errors'].keys()}")
            logger.info(
    ...
    logger.info(
        f"Finished creating windows. Duration: {duration} min for {len(labeled_files)} files")
