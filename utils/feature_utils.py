...
def expand_df(df: pd.DataFrame, anonymize_file: dict):
    ...
    if error:
        logger.error(
            f"Computing acceleration and gradients not possible for malformed df")
        return pd.DataFrame(), error

    # Append the overall acceleration
    df_with_diff_and_acc, error = extend_with_acceleration(
        df_with_diffs, anonymize_file)
    if error:
        return pd.DataFrame(), error
...
def extend_with_differences(df: pd.DataFrame, anonymize_file: dict):
    ...
    """
    differences_cols = anonymize_file.get(
        "feature_utils").get("differences_cols")

    if not set(differences_cols[0:2]).issubset(set(df.columns)):
        logger.error(
            f"Given df does not contain columns for the calculation of differences")
        return pd.DataFrame(), "differences_cols_not_found_calculation_of_diff_failed"

    df[differences_cols[2]] = abs(
        df[differences_cols[0]] - df[differences_cols[1]])
    return df, None
...
def extend_with_acceleration(df: pd.DataFrame, anonymize_file: dict):
    ...
    if not set(accel_cols[0:3]).issubset(set(df.columns)):
        logger.error(
            f"Calculation of the overall acceleration is not possible as the needed accel columns are not in the df")
...
def is_valid(df: pd.DataFrame, anonymize_file: dict):
    ...
    if not set(needed_cols).issubset(set(df.columns)):
        ...
        logger.error(
            f"Some needed columns are missing in the given df: {diff}")
        return "needed_cols_missing"""

    return None


def create_df_boundaries(df: pd.DataFrame, label: np.int64):
    """ Create a df with label, start- and endtime as entry
    """
    if not isinstance(label, np.int64):
        logger.error(f"No valid label given")
        return pd.DataFrame(), "given_label_not_int64"
    if not set(["time"]).issubset(df.columns):
        logger.error("Could not find a df time in given df")
    ...
    labels = pd.DataFrame(columns=['time_start', 'time_end', 'label'])
...
def feature_functions_dict(datapath: str):
    ...
    magnitude_area_cols: list = anonymize_file.get(
        "feature_utils").get("accel_cols")[0:3]
    magn_feat_name: str = anonymize_file.get(
        "feature_utils").get("magn_feat_name")[0]

    on_single_cols: list = anonymize_file.get(
        "feature_utils").get("on_single_column")
    on_single_cols.extend(
        [median_absolute_deviation, root_mean_square, variation_coefficient])
