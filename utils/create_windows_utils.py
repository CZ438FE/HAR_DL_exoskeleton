...
def prepare_time_col(df: pd.DataFrame, file: str):
    ...
    if not isinstance(df, pd.DataFrame):
        logger.error(
            f"Expected df of datatype pd.DataFrame, got {type(df)} for file {file}")
        return pd.DataFrame(), f"expected_pdDataFrame_got_{type(df)}"

    if not "time" in df.columns:
        logger.error(
            f"Malformed df given: Could not find a col time in file {file}")
    ...
    if not data_utils.has_identical_length(df, "time"):
        logger.info(
            f"Got df with varying degrees of precision in the time col for file {file} . Transform time col")
    ...
    if not data_utils.has_identical_length(df, "time"):
        logger.error(
            f"Df has varying degrees of precision after transforming time col in file {file}")
...
def create_list_of_windows(df: pd.DataFrame, window_length: int, step_length: int, file: str):
    ...
    # Create a list to store the starting and endpoints of the windows
    windows = []
    ...
    if windows == [] and seen_time_in_df > window_length:
        ...
        logger.error(
            f"Could not generate any windows for file {file}  Please investigate")
...
def handle_window(df: pd.DataFrame, window: list, df_filled: pd.DataFrame, data_columns: list, flatten: bool, file: str, window_length: int, resampling_rate: int, store_local: bool, saving_folder: str, windows_saved_current_file: int):
    """Saves the (flattened) window if it does contain unfilled data, updates amount of saved win and return an error message
    """
    if not isinstance(df, pd.DataFrame):
    ...
    if not isinstance(window[0], int) or not isinstance(window[1], int):
        logger.error(
            f"got wrong datatypes for the window_boundaries: expected int, got {[type(x) for x in window]}")
    ...
    first_time_index = data_utils.find_first_index_bigger_x_in_sorted_list(
        [x for x in df["time"]], window[0]-1)
    last_time_index = data_utils.find_first_index_bigger_x_in_sorted_list(
        [x for x in df["time"]], window[1])
    df_to_save = df.iloc[first_time_index:last_time_index,
    ...
    if err:
        logger.debug(
            f"Got a window with no real data within the df, skipped saving")
    ...
    # As the original df and the df_filled may contain differing rows and row numbers, a recalaculation is needed
    first_time_index = data_utils.find_first_index_bigger_x_in_sorted_list(
        [x for x in df_filled["time"]], window[0]-1)
    last_time_index = data_utils.find_first_index_bigger_x_in_sorted_list(
        [x for x in df_filled["time"]], window[1])
    df_to_save = df_filled.iloc[first_time_index:last_time_index, :].reset_index(
        drop=True)

    if flatten:
        df_to_save, error = data_utils.flatten(
            df_to_save, file, window_length, resampling_rate)
        if error:
    ...
    if store_local:
        ...
        new_filename = file.split(
            "/")[-1].replace(".csv", f"_{windows_saved_current_file +1}.csv")
...
def create_windows_for_file(file_with_attributes: dict):
    ...
    if list(df.columns) != needed_columns:
        logger.error(
            f"Columns of file {file} do not match the needed columns. Please investigate")
    ...
    if normalize == "z_normalize":
        df[data_columns] = data_utils.z_normalize_df(
            df[data_columns].copy(True), normalization_df)
    elif normalize == "max":
        df[data_columns] = data_utils.max_scale_df(
            df[data_columns], normalization_df)

    # Calculate the boundaries for the windows to create from the df and the given window_size and step_length
    windows, error = create_list_of_windows(
        df, window_length, step_length, file)
    if error:
        return data_utils.create_result_string(file, [1], 0, error)
    ...
    df_filled, error = time_utils.resample_data(
        df_filled, str(resampling_rate)+"ms")
    if error:
        return data_utils.create_result_string(file, [1], 0, error)
    ...
    for window in windows:
        success, windows_saved_current_file, error = handle_window(
            df, window, df_filled, data_columns, flatten, file, window_length, resampling_rate, store_local, saving_folder, windows_saved_current_file)
    ...
    logger.debug(
        f"Finished processing {windows_saved_current_file} windows for the file {file}")
...
def valid_attributes(labeled_files: list, needed_columns: list, window_length: int, step_length: int, filling_method: str, resampling_rate: int, data_columns: list, flatten: bool, store_local: bool, saving_folder: str, normalize: str):
    ...
    if not isinstance(labeled_files, list):
        logger.error(
            f"Got labeled_files {labeled_files} which is not a list, investigation needed")
    ...
    if not labeled_files:
        logger.error(
            f"Got empty list of labeled_files {labeled_files}, investigation needed")
        return "labeled_files_empty"

    if not isinstance(needed_columns, list):
        logger.error(
            f"Got needed_columns {needed_columns} which is not a list, investigation needed")
        return "needed_columns_not_a_list"

    if not needed_columns:
        logger.error(
            f"Got empty list of needed columns {needed_columns}, investigation needed")
        return "needed_columns_empty"

    if not isinstance(window_length, int):
        logger.error(
            f"Given window length is not an int: {window_length, type(window_length)}, investigation needed")
        return "non_int_window_length"

    if window_length < 10:
        logger.error(
            f"Received too small window length: {window_length}, investigation needed")
        return "window_length_too_small"

    if step_length < 2:
        logger.error(
            f"Received too small step_length: {step_length}, investigation needed")
        return "step_length_too_small"

    if step_length > window_length:
        logger.error(
            f"Given window_length {window_length} is smaller than given step_length {step_length} Not allowed to avoid loss of data")
        return "step_length_bigger_window_length"

    valid_filling_methods = ["ffill", "linear"]
    if filling_method not in valid_filling_methods:
        logger.error(
            f"Given filling_method {filling_method} not yet implemented choose from  {valid_filling_methods}")
        return "filling_method_not_implemented"

    if not isinstance(resampling_rate, int):
        logger.error(
            f"Gor resampling_rate of wrong data-type, Expected int, got {type(resampling_rate)} ")
    ...
    if not set(data_columns).issubset(needed_columns):
        logger.error(
            f"The data columns are not a true subset of the needed cols")
    ...
    if not isinstance(store_local, bool):
        logger.error(
            f"Expected type bool for store_local, got {type(store_local)}")
        return "got_store_local_of_nonbool_type"

    if not isinstance(saving_folder, str):
        logger.error(
            f"Expected type str for store_local, got {type(saving_folder)}")
...
def valid_table(table: dict):
    ...
    if not os.path.exists(table.get("data_path")):
        logger.error(
            f"Could not find folder datapath {table.get('data_path')}")
    ...
    if not os.path.exists(os.path.join(table.get("data_path"), "labeled_data")):
        logger.error(
            f"Could not find required folder labeled_data in {table.get('data_path')}")
    ...
    if not isinstance(table.get("dryrun"), bool):
        logger.error(
            f"Wrong datatype for dryrun given. Expected bool, got  {type(table.get('dryrun'))}")
        return "got_dryrun_of_nonbool_datatype"

    # Check if the windows lengths and step lengths are correct
    error = data_utils.valid_window_and_step_length(
        table.get("window_length"), table.get("step_length"))
    if error:
        return error
    ...
    if not isinstance(table.get("resampling_rate"), int):
        logger.error(
            f"Wrong datatype for resampling_rate given. Expected int, got  {type(table.get('resampling_rate'))}")
        return "got_resampling_rate_of_nonint_dataytpe"

    if table.get("resampling_rate") > 200:
        logger.error(
            f"Given resampling_rate {table.get('resampling_rate')} too big ")
        return "resampling_rate_too_big"

    if table.get('resampling_rate') < 2:
        logger.error(
            f"Given resampling_rate {table.get('resampling_rate')} too small ")
        return "resampling_rate_too_small"

    if table.get("resampling_rate") > table.get("window_length"):
        logger.error(
            f"Given resampling_rate {table.get('resampling_rate')} is bigger than specified window_length {table.get('window_length')}")
    ...
    if not isinstance(table.get('flatten'), bool):
        logger.error(
            f"Expected type bool for flatten, got {type(table.get('flatten'))}")
        return "got_flatten_of_nonbool_type"
    ...
    _, error = file_utils.get_files(os.path.join(
        table.get("data_path"), "labeled_data"), "csv")
    if error:
        return error
    ...
    if table.get("normalize") not in valid_normalization_levels:
        logger.error(
            f"Received invalid normalization level {table.get('normalize')}")
        return "invalid_normalization_level_given"
...
def return_normalization_df(normalize: str, anonymize_file: dict):
    ...
    if normalize == "z_normalize":
        normalization_df = pd.DataFrame(
            columns=anonymize_file["create_windows"]["data_columns"])
    ...
    logger.error(
        f"Could not return valid normalization df with unknown normalization type {normalize}")
