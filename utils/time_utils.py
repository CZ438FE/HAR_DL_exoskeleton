...
def transform_time_unit(df: pd.DataFrame, target_unit:str, time_col:str):
    ...
    if target_unit not in ["s", "ms"]:
        logger.error(
            f" Got invalid time unit {target_unit}, therefore no conversion possible")
        return pd.DataFrame(), f"invalid-target_unit_{target_unit}_given"

    # As the time is handeled in milliseconds, no transforming is needed if it already is in the correct time
    if target_unit == "s":
        final_len = 10
    elif target_unit == "ms":
        final_len = 13
    # Cannot generate a common factor of multiplication for the transformation, if the data existst in different time-related precision
    identical_length = len(str(df[time_col].max())) == len(
        str(df[time_col].min()))
    if not identical_length:
        logger.error(
            f"Cannot find common factor for multiplication, if input column has varying degrees of precision")
    ...
    df[time_col] = np.multiply(
        df[time_col].to_numpy(), 10**len_diff).astype(int)

    identical_length = len(str(df[time_col].max())) == len(
        str(df[time_col].min()))
    if not identical_length:
        logger.error(
            f"Something went wrong with the transformation, as the resulting time_col has varying degrees of precision")
...
def bring_col_to_greatest_len(df: pd.DataFrame, col: str):
    ...
    if not isinstance(df[col][0], np.int64):
        logger.error(
            f"Adjusting cardinality of col {col}, failed: wrong dtype given")
    ...
    while min_len < max_len:
        df[col] = np.where(df[col].to_numpy()*10 < df[col].max(),
                           df[col].to_numpy()*10, df[col].to_numpy())
        min_len = len(str(df[col].min()))
...
def error_in_time_col(df: pd.DataFrame, time_col: str):
    ...
    if datetime.fromtimestamp(min_time).year < 2018:
        logger.error(
            f"The earliest seen data comes from a year smaller than 2018: {datetime.fromtimestamp(min_time).year} Investigation needed")
    ...
    if datetime.fromtimestamp(max_time).year > 2023:
        logger.error(
            f"The latest seen data comes from a year greater than 2023: {datetime.fromtimestamp(max_time).year} Investigation needed")
    ...
    if (max_time-min_time) > eight_hours_in_s:
        logger.error(
            f"The seen session duration {(max_time-min_time/3600)}h is longer than 8hours. Investigation needed")
...
def transform_sci_not(df: pd.DataFrame, time_col: str):
    ...
    if sci_marker is None:
        logger.error(
            f"Time column is neither an int, nor in scientific notation, therefore no processing possible")
...
def resample_data(df: pd.DataFrame, sample_rate='10ms', time_unit='ms'):
    ...
    if not isinstance(df["time"][0], np.int64):
        logger.error(
            f"received time col with non-int dt: {type(df['time'][0])}")
    ...
    try:
        ...
        logger.error(
            f"Could not generate a valid duration for resampling from {sample_rate}")
        return pd.DataFrame(), f"generating_valid_sampling_dur_from_{sample_rate}_failed"
    if resampling_int < 2:
        logger.error(
            f"The given resampling rate is too small: {resampling_int}. Must be greater 1")
        return pd.DataFrame(), "sampling_rate_smaller_two_given"
    if resampling_int > 100:
        logger.error(
            f"The given resampling rate is too big: {resampling_int}. Max: 100")
    ...
    df_resampled = df_resampled.astype(
        {"time": np.int64}).reset_index(drop=True)
...
def valid_time_string(time_string: str):
    ...
    if not isinstance(time_string, str):
        logger.error(
            f"Wrong datatype for the time_string given. Expected str, got {type(time_string)}")
        return "got_non_str_time_string"
    # Check if the string has the wanted length
    if len(time_string) != 16:
        logger.error(
            f"Got a string of an invalid length, expected 16 , got  {len(time_string)}. Wanted format: YYYY-mm-DDTHH:MM")
        return "got_time_str_of_invalid_len"
    # Check if the string contains the correct amount of separators
    if time_string.count("-") != 3:
        logger.error(
            f"Expected 3 - as separators in the time_string. got {time_string.count('-')}")
        return "wrong_amount_of_-_separators_in_time_string"
    if time_string.count("T") != 1:
        logger.error(
            f"Expected 1 T as separator in the time_string. got {time_string.count('T')}")
