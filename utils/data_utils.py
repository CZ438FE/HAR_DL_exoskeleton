...
def only_nan(df: pd.DataFrame, cols: list):
    """Checks if the df has only NaN or 0 in the given columns
    """
...
def valid_window_and_step_length(window_length: int, step_length: int, min_window_length=1, min_step_length=1):
    ...
    if window_length is None:
        logger.error(f"Cannot create features with no window_length given")
        return "window_length_is_None"
    # Check if a step length was given
    if step_length is None:
        logger.error(
            f"Cannot create features with no step_length given: {step_length}")
        return "step_length_is_None"
    # Check if the window length is valid
    if window_length < min_window_length:
        logger.error(
            f"Cannot create windows of a length shorter than the minimum window length {min_window_length}. Choose bigger windows size")
        return f"given_window_length_smaller_{min_window_length}"
    # Check if the step length is valid
    if step_length < min_step_length:
        logger.error(
            f"Cannot create windows by using a shorter step length than the minimum step length {min_step_length}. Choose bigger step size")
    ...
    if step_length > window_length:
        logger.error(
            f"Given step length {step_length} is greater than specified window_length. This is not allowed, to prevent loss of information. Reduce the step_length")
...
def create_filled_df(df: pd.DataFrame, filling_method: str):
    ...
    if filling_method not in valid_filling_methods:
        logger.error(
            f"The given filling method {filling_method} is not implemented/valid. Choose from {valid_filling_methods}")
        return pd.DataFrame(), f"invalid_filling_method_{filling_method}_given"

    if not isinstance(df, pd.DataFrame):
        logger.error(
            f"Got invalid datatype for the df. Expected Pandas Df, got {type(df)}")
        return pd.DataFrame(), f"expected_pdDataFrame_got_{type(df)}"

    if "time" not in df.columns:
        logger.error(
            f"Df without a time column given, constructing of df_filled not possible")
        return pd.DataFrame(), "time_col_missing"

    if "label" not in df.columns:
        logger.error(
            f"Df without a label column given, constructing df_filled not possible")
        return pd.DataFrame(), "label_col_missing"

    if len(df["label"].unique()) > 1:
        logger.error(f"Got df with multiple label levels")
    ...
    df_to_be_filled = df_to_be_filled.sort_values(
    ...
    step_length_in_ms = 1
    all_potential_timepoints = set(
        [x for x in range(earliest_timepoint, last_timepoint, step_length_in_ms)])
    all_seen_timepoints = set([x for x in df_to_be_filled["time"]])

    all_timepoints_to_add = list(
        all_potential_timepoints.difference(all_seen_timepoints))
    ...
    df_filled = df_filled.sort_values("time", ascending=True).reset_index(
        drop=True).astype({"time": np.int64})
    ...
    if filling_method == "ffill":
        ...
        logger.error(
            f"Received filling method {filling_method}, which is implemented yet")
        return pd.DataFrame(), f"invalid_filling_method_{filling_method}_given"
...
def flatten(df_to_flatten: pd.DataFrame, file: str, window_length=1000, resampling_length_ms=10, remove_time_entries=True):
    ...
    if not "time" in df_to_flatten.columns or "label" not in df_to_flatten.columns:
        logger.error(
            f"The given df does not contain a time a label column. Investigation needed for file {file}")
    ...
    if len(df_to_flatten.columns) != 11:
        logger.error(
            f"Received wrong amount of columns. Expected 11, got {len(df_to_flatten.columns)} at file {file} ")
    ...
    if len(df_to_flatten) != (window_length/resampling_length_ms):
        logger.error(
            f"Received df with the wrong len. Expected a len of {(window_length/resampling_length_ms)}, got len {len(df_to_flatten)}. file {file}")
...
def downsize_label(label_series: pd.Series, label_depth: int):
    ...
    if label_depth not in valid_label_depths:
        logger.error(
            f"Received invalid label_depth {label_depth}. Choose from {valid_label_depths}")
    ...
    if len(str(label_series.min())) < label_depth:
        logger.error(
            f"Received label column with a granularity which is smaller than the needed label_depth {label_depth}. Investigation needed")
    ...
    cutted_label = np.where(label_series.to_numpy(
    ) < 100, label_series.to_numpy(), label_series.to_numpy()/10).astype(int)
    if label_depth == 1:
...
def prepare_label(label_series: pd.Series, label_depth: int, label_mapping=None):
    ...
    if label_depth not in valid_label_depths:
        logger.error(
            f"The given label_depth {label_depth} is not in the valid_label_depths {valid_label_depths}")
...
def label_class_mean(df: pd.DataFrame, col: str, label_col: str):
    ...
    if label_col not in df.columns:
        logger.error(
            f"Cannot calculate labelwise mean without label col {label_col}")
    ...
    for df in list_of_dfs:
        for col in df.columns:
            ...
            if df[col].isna().any():
                ...
                # calculate a classwise mean for this row
                col_means, error = label_class_mean(df, col, label_col)
                if error:
                    return pd.DataFrame(), error

                col_without_nas = [col_means[df.iloc[number, label_idx]] if math.isnan(
                    x) else x for number, x in enumerate(df[col])]
...
def naively_impute_if_needed(all_data_df: pd.DataFrame, log_file: dict):
    ...
    unexpected_na_values_found, error = unexpected_na_values_seen(
        all_data_df, log_file)
    if error:
        return pd.DataFrame(), error

    if unexpected_na_values_found:
        logger.info(f"Detected nan values, beginning with naive imputation")
...
def unexpected_na_values_seen(df: pd.DataFrame, log_file: dict):
    ...
    if "reduce_data" not in log_file.keys():
        logger.error(
            f"processing featured data without reducing the data is not allowed")
...
def unexpected_na_in_hierarchical_features(df: pd.DataFrame, log_file: dict):
    ...
    for individual_df in list_of_dfs:
        if individual_df.isna().any().any():
            ...
            proportion_of_nan_values = nr_of_nan_values / \
                (len(individual_df) * len(individual_df.columns))

            if proportion_of_nan_values > 0.1:
                logger.error(
                    f"The amount of data to impute is too big: {proportion_of_nan_values*100} % of all available data")
...
def create_top_and_mid_df(df: pd.DataFrame, log_file: dict):
    ...
    if "generate_features" not in log_file.keys():
        logger.error(f"creating sub_dfs is only valid for featured data")
    ...
    if "needed_columns" not in log_file["reduce_data"].keys():
        logger.error(f"log_file[reduce_data] does not contain the needed cols")
    ...
    top_level_df = create_top_level_df(
        internal_copy_df, log_file, df_resting_data)

    mid_level_df = create_mid_level_df(
        df, internal_copy_df, log_file, top_level_df, df_resting_data)
...
def create_list_of_dfs_for_imputation(df: pd.DataFrame, log_file: dict):
    ...
    if "reduce_data" not in log_file.keys():
        logger.error(f"Imputation is not allowed for unreduced data")
...
def join_list_of_sub_dfs(list_of_subdfs: list):
    ...
    for df in list_of_subdfs:
        if not isinstance(df, pd.DataFrame):
            logger.error(f"detected element of non pd.Dataframe type")
...
def prepare_label_for_featured_data(df: pd.DataFrame, label_depth: int):
    ...
    if label_depth not in valid_label_depths:
        logger.error(f"Received invalid label_depths")
...
def prepare_labels_hierarchical_classification(df: pd.DataFrame, label_depth: int):
    ...
    if label_depth < 2:
        logger.error(
            f"Cannot prepare for hierarchical classification, when top-level classification is requested")
        return pd.DataFrame(), "received_too_small_label_depth"

    if len(str(df["label"].min())) != len(str(df["label"].max())):
        logger.error(
            f"Cannot prepare_label for hierarchical class for unequal label precision")
        return pd.DataFrame(), "unequal_label_precision_found"

    if len(str(df["label"].min())) != 2:
        logger.error(
            f"Cannot prepare_label for hierarchical class. for precision != 2")
    ...
    joined_df = joined_df.sort_values("index_col").reset_index(
        drop=True).drop(columns=["index_col"])

    return joined_df, error


def rowwise_proportion_of_na_smaller_than_boundary(df: pd.DataFrame, proportion: float):
...
def split_joined_mid_level_df(df: pd.DataFrame, log_file: dict):
    ...
    for top_level_label in sorted(df["label_top"].unique()):
        ...
        mid_level_df = df[df["label_top"] ==
                          top_level_label][needed_columns].reset_index(drop=True)
...
def create_top_level_df(df: pd.DataFrame, log_file: dict, df_top_level_without_subcategories: pd.DataFrame):
    ...
    """
    top_level_features = log_file["reduce_data"]["needed_columns"][0].copy()

    top_level_features.extend(["label_top", "label_mid", "index_col"])

    top_level_df = df[top_level_features].copy(True)
    top_level_df["top_level_marker"] = rowwise_proportion_of_na_smaller_than_boundary(
        top_level_df, 0.5)
    top_level_df = top_level_df[top_level_df["top_level_marker"]]

    top_level_features.extend(["top_level_marker"])
...
def create_mid_level_df(df: pd.DataFrame, reduced_copy_df: pd.DataFrame, log_file: dict, top_level_df: pd.DataFrame, df_top_level_without_subcategories: pd.DataFrame):
    ...
    reduced_copy_df = pd.concat([reduced_copy_df, df_top_level_without_subcategories]).sort_values(
        "index_col").reset_index(drop=True)
    mid_level_df = reduced_copy_df[needed_mid_level_cols].reset_index(
        drop=True)

    # Calculate the indices for the mid-level data, index by it and set the top-level marker to False
    mid_level_row_indices = sorted(
        list(set(df["index_col"]).difference(set(top_level_df["index_col"]))))
...
def determine_standardization_type(standardizing_df: pd.DataFrame):
    ...
    logger.error(
        f"Could not determine standardization_type of given standardization file")
...
def bring_standardizing_df_in_correct_order(standardizing_df: pd.DataFrame, hierarchical_model: str):
    ...
    if hierarchical_model == "lifting":
        standardizing_df.columns = [
            x for x in range(len(standardizing_df.columns))]
        rearrannged_df = pd.DataFrame(
            standardizing_df[[0, 4, 1, 2, 3, 5, 6]].to_numpy())
    ...
    if hierarchical_model == "walking":
        standardizing_df.columns = [
            x for x in range(len(standardizing_df.columns))]
        rearrannged_df = pd.DataFrame(
            standardizing_df[[0, 2, 1, 3, 4, 5, 6, 7]].to_numpy())
        return rearrannged_df, None
...
def prepare_standardization(model_folder: str, standarization_needed: bool, hierarchical_model: str):
    ...
    """
    standardizing_df, standardization_type, error = prepare_standardization_df(
        model_folder, standarization_needed)
    if error:
        return None, None, error

    # Ensure that the order of the cols is correct before applying the standardization
    standardizing_df, error = bring_standardizing_df_in_correct_order(
        standardizing_df, hierarchical_model)
    if error:
        return None, None, error
