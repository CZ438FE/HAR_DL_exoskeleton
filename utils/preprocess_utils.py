...
def return_svm_features(label_depth: int, data_path: str):
    ...
    if label_depth not in [1, 2]:
        logger.error(
            f"Cannot return the needed features when an invalid label_depth is given")
    ...
    if label_depth == 2:
        # Additionally keep the features for the subactivity-lifting analyzer
        needed_columns.extend(
            [anonymize_file.get("svm_features").get("lifting")])

        # Additionally keep the features for the subactivity-walking analyzer
        needed_columns.extend(
            [anonymize_file.get("svm_features").get("walking")])
...
def update_label_distributions(label_dict: dict, df: pd.DataFrame, label_depth: int):
    ...
    if "label" not in df.columns:
        logger.error(
            "Counting the labels is not possible when df does not contain a column label")
    ...
    for i in range(len(label_appearances)):
        ...
        if len(key_in_label_dict) < label_depth and label_depth != 3:
            logger.error(
                f"Got label of inappropriate granularity: Expected depth {label_depth}, got {len(key_in_label_dict)}")
            return {}, "label_of_inappropriate_granularity_found"
        # if no data of this label has been seen yet, create a new key
        if key_in_label_dict not in label_dict.keys():
            label_dict[key_in_label_dict] = label_appearances.iloc[i]
        # Otherwise simply update the value of the key
        else:
            label_dict[key_in_label_dict] += label_appearances.iloc[i]
...
def update_classes_files(label_dict: dict, df: pd.DataFrame, label_depth: int, file: str):
    ...
    for i in range(len(label_appearances)):
        if str(label_appearances.index[i])[:label_depth] not in label_dict.keys():
            ...
            label_dict[str(label_appearances.index[i])
                       [:label_depth]].append(file)
    return label_dict, None
...
def create_labels_overview_dict(file_with_params: dict):
    ...
    if label_depth not in [1, 2, 3]:
        logger.error(f"received invalid label_depth")
        return "invalid_label_depth_given"
    ...
    classes_to_files_dict["classes_count"], error = update_label_distributions(
        classes_to_files_dict["classes_count"], df, label_depth)
    if error:
        return error

    classes_to_files_dict["classes_files"], error = update_classes_files(
        classes_to_files_dict["classes_files"], df, label_depth, file)
    if error:
        return error
...
def calculate_needed_extra_samples(labels_overview_dict: dict, method: str, hierarchical: bool):
    ...
    """
    ref_class = find_reference_class(
...
def calculate_differences(ref_class: list, labels_overview_dict: dict):
    ...
    if ref_class == []:
        logger.error(
            f"Calculation of differences not possible when the reference class is undefined")
        return {}

    if not labels_overview_dict:
        logger.error(
            "Cannot calculate the differences when the dict with labels and their appeareance is empty")
        return {}

    differences = {}
    for key in labels_overview_dict["classes_count"].keys():
        differences[key] = ref_class[1] - \
            labels_overview_dict["classes_count"][key]
    return differences
...
def adjust_granularity_of_label(df: pd.DataFrame, colname: str, label_depth: int):
    ...
    if label_depth not in [1, 2]:
        logger.error(
            f"Adjusting granularity for this granularity not implemented yet")
        return None, "invalid_label_depth given"

    cutted_label = np.where(df[colname].to_numpy(
    ) < 100, df[colname].to_numpy(), df[colname].to_numpy()/10).astype(int)
    if label_depth == 1:
...
def calculate_prop_original_data_dict(needed_samples: dict, labels_overview_dict: dict):
    ...
    if not isinstance(needed_samples, dict) or not isinstance(labels_overview_dict, dict):
        logger.error(
            f"Got invalid data given, got needed_samples of type {type(needed_samples)} and labels_overview of type {type(labels_overview_dict)} . Expected dicts")
        return {}
    if not set(needed_samples.keys()).issubset(labels_overview_dict["classes_count"].keys()):
        logger.error(
            f"The keys of needed_samples and the classes observed in the labels_overview_dict differ.")
        return {}
    ...
    for key in sorted([int(x) for x in needed_samples.keys()]):
        ...
        prop_exis = labels_overview_dict.get("classes_count").get(str(
            key)) / (labels_overview_dict.get("classes_count").get(str(key)) + needed_samples[str(key)])
        # Append the calculated proportions and the respective class to the dict
        prop_original_data_dict.get(
            "prop_existing_after_balancing").append(round(prop_exis, 2))
...
def dangerously_much_oversampling_needed(prop_original_data_dict: dict, warning_boundary=0.9):
    ...
    if not isinstance(prop_original_data_dict, dict):
        logger.error(
            f"Malformed prop_original_data_dict given: Expected dict, got {type(prop_original_data_dict)} ")
        return
    if not set(["prop_existing_after_balancing"]).issubset(prop_original_data_dict.keys()):
        logger.error(
            f"Malformed prop_original_data_dict given: key prop_existing_after_balancing not found")
        return
    ...
    for i in range(len(prop_original_data_dict.get("prop_existing_after_balancing"))):
        if prop_original_data_dict.get("prop_existing_after_balancing")[i] < warning_boundary:
            logger.warning(
                f"Dangerously much resampling needed for class {prop_original_data_dict.get('class')[i]} . More sophisticated Resampling might be necessary")
            return True
...
def naive_oversampling(needed_samples: dict, key: str, df_to_resample: pd.DataFrame, seed=42):
    ...
    while len(df_to_resample) < optimal_length:
        ...
        if optimal_length >= (len(df_to_resample)*2):
            ...
            factor = int((needed_samples.get(key) +
                         len(df_to_resample))/len(df_to_resample))

            df_to_resample = pd.concat(
                [df_to_resample]*factor).reset_index(drop=True)
        else:
            random_indexes = draw_random_indexes(
                optimal_length, len(df_to_resample), seed)

            # Append the drawn rows to the df
            df_to_resample = pd.concat(
                [df_to_resample, df_to_resample.iloc[random_indexes]]).reset_index(drop=True)
        ...
        if iteration > 15:
            logger.error(
                f"Oversampling not finished after {iteration} iterations. Investigation needed.")
...
def perform_naive_oversampling(key_with_params: dict):
    ...
    list_of_filchunks, error = file_utils.list_of_maxsized_filechunks(
        all_files_to_read_in, 0.5)
    if error:
    ...
    needed_samples_for_chunk = {key: math.ceil(
        needed_samples[key]/len(list_of_filchunks))}
    ...
    for number, chunk in enumerate(list_of_filchunks):
        ...
        df_to_resample, error = adjust_granularity_of_label(
            df_to_resample, "label", label_depth)
        if error:
        ...
        if len(df_to_resample["label"].unique()) > 1:
            logger.error(
                f"found multiple labels in the chunk: {df_to_resample['label'].unique()}")
        ...
        if store_local and number + 1 == len(list_of_filchunks):
            needed_samples_for_chunk = {
                key: needed_samples[key] - saved_needed_samples}

        # oversample the data
        df_resampled, error = naive_oversampling(
            needed_samples_for_chunk, key, df_to_resample, seed)
        if error:
            return [data_utils.create_result_string(chunk[0], [1], saved_files, error)]
        ...
        if store_local:
            ...
            # Save each resampled_df under an own file
            error = file_utils.save_df_with_key(
                chunk[0], saving_folder, df_resampled, key)
            if error:
                return data_utils.create_result_string(chunk[0], [1], saved_files, error)
            saved_needed_samples += (len(df_resampled) -
                                     nr_samples_before_oversampling)

    # As all files of the given key have been processed successfully, this needs to be reflected in the list_of_result strings
    list_of_result_strings = [data_utils.create_result_string(successfully_processed_file, [
                                                              None], 1, "None") for successfully_processed_file in labels_overview_dict["classes_files"][key]]
        ...
        for key in labels_over_view_dict:
            ...
            for subkey in labels_over_view_dict[key]:
                if subkey not in overall_labels_overview_dict[key].keys():
                    ...
                    if isinstance(overall_labels_overview_dict[key][subkey], np.int64):
                        overall_labels_overview_dict[key][subkey] += labels_over_view_dict[key][subkey]
                    else:
                        overall_labels_overview_dict[key][subkey].append(
                            labels_over_view_dict[key][subkey][0])
...
def reduce_data_of_file(file_with_params: dict):
    ...
    df_to_save, error = correct_format_for_multilevel_classification(
        df, needed_columns)
    if error:
...
def count_label_distribution(all_files: list, label_depth: int):
    ...
    for chunk_number, chunk in enumerate(file_chunks, start=1):
        if chunk_number % 10 == 0 or chunk_number == 1:
            logger.info(
                f"[{chunk_number}/{len(file_chunks)}] Started counting label in chunk")

        with Pool() as pool:
            result = pool.map(create_labels_overview_dict, chunk)

        labels_overview_dict = update_overall_labels_overview_dict(
            result, labels_overview_dict)
    ...
    if label_depth == 2:
        if "32" in labels_overview_dict["classes_count"].keys():
            labels_overview_dict["classes_count"]["31"] += labels_overview_dict["classes_count"].pop(
                "32")
            labels_overview_dict["classes_files"]["31"] += labels_overview_dict["classes_files"].pop(
                "32")
        ...
        for key in labels_overview_dict:
            for subkey in labels_overview_dict[key]:
                if str(subkey).startswith("1"):
                    ...
                    logger.error(
                        f"received invalid subkey {subkey}. No submodel exists for this more_general_class")
...
def create_list_of_names(folder: str, prefix: str, process: str, file_ending: str, length_of_list: int, hierarchical_featured: bool, list_of_specific_prefixes=None):
    """creates a list of names of the form <folder>/<prefix>_<process>_<uuid>.file_ending
    """

    if hierarchical_featured:
        list_of_specific_prefixes = [
            int(str(prefix)[0]) if x % 2 == 0 else prefix for x in range(length_of_list)]
        uuid_list = [str(uuid.uuid4())
                     for x in range(len(list_of_specific_prefixes))]
        uuid_list_full_length = list(itertools.chain.from_iterable(
            itertools.repeat(x, 2) for x in uuid_list))
        return [os.path.join(folder, f"{list_of_specific_prefixes[x]}_{process}_{uuid_list_full_length[x]}.{file_ending}") for x in range(length_of_list)], None
    ...
    if list_of_specific_prefixes:
        ...
        return [os.path.join(folder, f"{list_of_specific_prefixes[x]}_{process}_{str(uuid.uuid4())}.{file_ending}") for x in range(length_of_list)], None
    return [os.path.join(folder, f"{prefix}_{process}_{str(uuid.uuid4())}.{file_ending}") for x in range(length_of_list)], None


def save_in_subfiles(df_with_params: dict):
    """Uses a df, to save the inidvidual rows as csv file
    """
    ...
    convolutional: bool = df_with_params.get("convolutional")
    ...
    list_of_names, error = create_list_of_names(
        saving_folder, label, "prepared", "csv", len(df), hierarchical_featured, None)

    if error:
        return data_utils.create_result_string("some_file", [1], saved_windows, error)
    [save_file(df.iloc[i, :], convolutional, list_of_names[i],
               hierarchical_featured) for i in range(len(df))]
    saved_windows += len(df)

    return [data_utils.create_result_string(f"{name}", [], saved_windows, str(error)) for name in list_of_names]


def handle_labels_df(result_list: list, df_labels: pd.DataFrame, seperator="+++"):
    """handles the result_list to create the the labels.csv 
    """
    filenames = []
    labels = []

    result_list = file_utils.unpack_list_of_result_strings_if_needed(
        result_list)

    for result in result_list:
        result_string_splitted = result.split(seperator)
        success = True if result_string_splitted[1] == "True" else False
        if success:
            filenames.append(result_string_splitted[0])
            labels.append(
                int(result_string_splitted[0].split("/")[-1].split("_")[0]))
        else:
...
def prepare_folder_for_custom_loading(folder: str, table: dict, log_file: dict, method="prepare_dataset", convolutional=False):
    ...
    processing_hierarchical_featured_data = "generate_features" in log_file.keys(
    ) and table.get("granularity") == "mid"

    # Handle each file after the other (slower, but may save RAM)
    for number, file in enumerate(all_unprepared_files, start=1):

        df = file_utils.read_csv_safely(file, header=None)
        list_of_sub_dfs = split_into_file_chunks(
            df, processing_hierarchical_featured_data, table)

        label = correct_label_for_saving_prepared_file(
            file, processing_hierarchical_featured_data, df)

        list_of_dfs_and_savingplace = [{"df": sub_df,
                                        "saving_folder": folder,
                                        "label": label,
                                        "convolutional": convolutional,
                                        "hierarchical_featured": processing_hierarchical_featured_data} for sub_df in list_of_sub_dfs]
        del list_of_sub_dfs
        ...
        log_file = file_utils.create_log_from_result_strings(
            result, os.path.join(folder, "log.json"), method)
        if len(all_unprepared_files) > 1:
            logger.info(
                f"[{number}/{len(all_unprepared_files)}] files finished. {len(log_file[method]['successfully_processed_files'])} files successfully processed")
        ...
        if len(log_file[method]["occurred_errors"]) != 0:
            ...
            logger.info(
                f"For debugging purposes the log file gets saved: Contains errors & incorrect files")
            # Update the log file
    ...
    labels_df = labels_df.sample(
        frac=1, random_state=table.get("seed")).reset_index(drop=True)
...
def remove_processed_key_with_params(keys_with_params: list, log_file: dict, method: str):
    ...
    """
    already_handeled_files = log_file[method]["successfully_processed_files"]

    key_with_params_to_remove = []
    for key_with_params in keys_with_params:
        files_to_handle_for_this_key = key_with_params.get("labels_overview_dict").get(
            "classes_files").get(key_with_params.get("key"))
        if set(files_to_handle_for_this_key).issubset(set(already_handeled_files)):
...
def valid_table(table: dict):
    ...
    if not os.path.exists(table.get("data_path")):
        logger.error(
            f"Given data_path {table.get('data_path')} does not exist")
    ...
    if not os.path.exists(table.get("input_data")):
        logger.error(f"input_data folder does not exist")
    ...
    if table.get("method") == "reduce_data":
        ...
        if not file_utils.processing_featured_data(log_file):
            logger.error(
                f"Reducing set of features not possible when input data does not exist in featured form")
            return "got_non_featured_input_data_for_method_reduce_data"

    if table.get("method") == "prepare_dataset":
        with open(os.path.join(table.get("input_data"), "log.json")) as f:
            log_file = json.load(f)
        if not file_utils.processing_oversampled_data(log_file):
            logger.error(
                f"Preparing dataset currently only implemented for already balanced data")
            return "got_unbalanced_set_for_training"

    if not isinstance(table.get("convolutional"), bool):
        logger.error(
            f"Got convolutional of type {type(table.get('convolutional'))}, expected bool")
        return "got_convolutional_of_non_bool_dt"
...
def handle_data_for_feature_based_ffnn(input_data_folder: str, saving_folder: str, method: str, log_file: dict, label_depth: int, table: dict, store_local: bool):
    ...
    """
    all_unprepared_files, error = file_utils.get_files(
        input_data_folder, "csv")
    if error:
        logger.error(f"The input data folder does not contain any files")
        return error
    # Check if the data sizes are valid
    size_of_all_files, error = file_utils.count_filesizes_of_dir(
        input_data_folder)
    if error:
        return error
    if size_of_all_files > 4:
        logger.error(
            f"received unexpectedly large amount of featured data in input_folder")
        return

    all_unprepared_files, error = file_utils.reduce_files_to_handle(
        all_unprepared_files, saving_folder, method)
    if error or all_unprepared_files == []:
        return error

    # use multiprocessing to read_in all the oversampled files safely
    with Pool() as pool:
    ...
    logger.info(
        f"These were the original labels of the currently processed files: {original_labels}")

    # Create the new labels: prepare_label for usage in train_models: Map meaningful label nrs to ints in range(1, nr_classes+1)
    all_data_df, error = data_utils.prepare_label_for_featured_data(
        all_data_df, label_depth)
    if error:
        return error

    all_data_df, error = data_utils.naively_impute_if_needed(
        all_data_df, log_file)
    if error:
        return error

    if "index_col" in all_data_df.columns:
        all_data_df = all_data_df.sort_values(
    ...
    logger.info(
        f"Saved the prepared_df into labelwise files, proceeding to save featured windows as individual files")

    df_labels, log_file, error = prepare_folder_for_custom_loading(
        saving_folder, table, log_file, method, False)
    if error:
        return error

    # Shuffle, to ensure that the resulting batches do not contain clumped data
    labels_df = df_labels.sample(
        frac=1, random_state=table.get("seed")).reset_index(drop=True)

    labels_df.to_csv(os.path.join(saving_folder, "labels.csv"), index=False)
    logger.info(f"Successfully saved rows of each label as individual files")
...
def prepare_dataset_for_flattened_data(input_data_folder: str, saving_folder: str, method: str, log_file: dict, label_depth: int, table: dict, store_local: bool, convolutional: bool):
    """does the heavy lifting to handle the files for training a NN on the flattened data (prepares either for FFNN or CNN depending on convolutional)
    """
    all_unprepared_files, error = file_utils.get_files(
        input_data_folder, "csv")
    if error:
    ...
    original_labels, error = file_utils.get_unique_levels_from_folder(
        input_data_folder, "label", np.int16, "csv")
    if error:
        return error
    log_file[method]["original_labels"] = original_labels
    logger.info(
    ...
    _, error = file_utils.estimate_needed_time(len(all_unprepared_files), method, os.cpu_count(
    ), file_utils.count_filesizes_of_list(all_unprepared_files))
    if error:
        return error
    ...
    for number, file in enumerate(all_unprepared_files, start=1):
        ...
        # Prepare the label col
        df["label"], error = data_utils.prepare_label(
            df["label"], label_depth, label_remapping)
        if error:
            return error
    ...
    logger.info(
        f"Saving the files labelswise finished, starting to save windowwise")
    df_labels, log_file, error = prepare_folder_for_custom_loading(
        saving_folder, table, log_file, method, convolutional)
    if error:
        return error
...
def return_df_channelwise_if_needed(df: pd.DataFrame, convolutional: bool, nr_channels=9):
...
def correct_format_for_multilevel_classification(df: pd.DataFrame, needed_columns: list):
    ...
    if label.startswith("1"):
        ...
        logger.error(
            f"Got a file with a label neither starting with 1 or 2 for mid-level classification")
        return pd.DataFrame(), "unexpected_label_seen"

    if len(feature_list_top_level) - len(feature_list_second_level) < 0:
        logger.error(f"received more features for sub-level analyzer ")
    ...
    final_df = pd.concat([first_row, second_row],
...
def split_into_file_chunks(df: pd.DataFrame, processing_hierarchical_featured_data: bool, table: dict):
    """As the filechunks need to be handeled differently for hierarchical data and unhierarchical, the chunks need to be made accordingly
    """
...
def save_file(col: pd.Series, convolutional: bool, name: str, hierarchical_featured: bool):
    """
    """
    ...
    return_df_channelwise_if_needed(col, convolutional).to_csv(
...
def calculate_needed_extra_samples_hierarchical_data(labels_overview_dict: dict):
    ...
    for label in labels_overview_dict["classes_count"].keys():
        caused_samples_dict[label] = labels_overview_dict["classes_count"][label] * \
            len(dict_of_analyzer_levels[label[0]]["subkeys"])

    max_samples = max([caused_samples_dict[key]
                      for key in caused_samples_dict.keys()])
    if max_samples % 2 == 1:
        max_samples += 1

    needed_samples = {}
    for label in labels_overview_dict["classes_count"].keys():
        needed_samples[label] = int((max_samples / len(dict_of_analyzer_levels[label[0]]
                                    ["subkeys"])) - labels_overview_dict["classes_count"][label])
...
def valid_hierarchical_oversampling_seen(label_series: pd.Series):
    """Tests if all the hierarchical data got oversampled correctly, meaning that each model to be trained has balanced data available
    """
    hierarchical_dict_of_samples = create_hierarchical_dict_of_samples(
        label_series)
    for key in hierarchical_dict_of_samples.keys():
        if key == "top_level":
            seen_occurences = [hierarchical_dict_of_samples[key][subkey]
                               for subkey in hierarchical_dict_of_samples[key].keys()]
            if len(set(seen_occurences)) != 1:
                logger.error(
                    f"The top-level labels do not occur in the same (balanced) frecquency : {hierarchical_dict_of_samples[key]}")
                return False
            continue
        for subanalyzer in hierarchical_dict_of_samples[key].keys():
            seen_occurences = [hierarchical_dict_of_samples[key][subanalyzer][subkey]
                               for subkey in hierarchical_dict_of_samples[key][subanalyzer].keys()]
            if len(set(seen_occurences)) != 1:
                logger.error(
                    f"The data for the subanalyzer {subanalyzer} not occur in the same (balanced) frecquency : {hierarchical_dict_of_samples[key][subanalyzer]}")
                return False
...
def create_hierarchical_dict_of_samples(label_series: pd.Series):
    ...
    if len(str(max(label_series.unique()))) != len(str(min(label_series.unique()))):
        logger.error(
            f"received label series with different set of granularities")
...
def draw_random_indexes(optimal_length: int, existing_length: int, seed: int):
    ...
    if optimal_length - existing_length >= 2:
        random_indexes = np.unique(np.random.randint(
            int(existing_length/2), size=int((optimal_length - existing_length)/2)))*2
        random_indexes = np.concatenate(
            [random_indexes, np.add(random_indexes, 1)])
