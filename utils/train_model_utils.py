...
def valid_table(table: dict):
    ...
    """
    if table.get("dryrun"):
        logger.warning(f"Storing results of training is highly recommended")

    # Check the folder of trainings_data
    if not os.path.exists(table.get("training_data")):
        logger.error(
            f"Given folder with training_data does not exist: {table.get('training_data')}")
    ...
    if table.get("validation_data") is not None:
        if not os.path.exists(table.get("validation_data")):
            logger.error(
                f"Given folder with validation data does not exist {table.get('validation_data')}")
    ...
    if table.get("validation_data") == table.get("training_data"):
        logger.error(
            f"Cannot use the same files as trainings-and validation data")
    ...
    if table.get("output_date"):
        location_future_log_file = os.path.join(table.get(
            "data_path"), "train_model", table.get("type"), table.get("output_date"), "log.json")
        if os.path.exists(location_future_log_file):
            ...
            if "already_trained_epochs" in log_file.get("train_model").keys():
                if log_file.get("train_model").get("already_trained_epochs") >= table.get("epochs"):
                    ...
                    logger.error(
                        f"Tried to train a model after training was already finished")
                    return "training_already_finished"

    if table.get("minibatch_size") < 1:
        logger.error(
            f"Minibatchsizes < 1 are not allowed. please choose a bigger minibatch_size ")
        return "minibatch_size_too_small"

    if table.get("epochs") < 0:
        logger.error(
            f"epochs < 0 are not allowed. please choose a bigger epochs ")
    ...
    error = valid_layerstring(table.get("layer_structure"), table.get(
        "type"), table.get("training_data"))
    if error:
        return error
    ...
    if table.get("dropout_rate") is not None:
        if not ((table.get("dropout_rate") >= 0) & (table.get("dropout_rate") < 1)):
            logger.error(
                f"Given dropout rate {table['dropout_rate']} is not within the allowed interval ]0,1[")
            return "dropout_rate_out_of_allowed_boundaries"

    if not ((table.get("learning_rate") > 0) & (table.get("learning_rate") < 1)):
        logger.error(
            f"Given learning_rate {table['learning_rate']} is not within the allowed interval ]0,1[")
...
def valid_layerstring(layerstring: str, network_type: str, folder: str):
    ...
    if network_type == "FFNN":
        ...
        return error
    if network_type == "CNN":
        conv_and_pool_layers, ff_layers, error = split_layerstring_into_conv_and_ff_layers(
            layerstring)
        if error:
            return error
    ...
    logger.error(
        f"String structure for other network types not implemented yet")
...
def valid_feedforward_layers(layers: list):
    ...
    try:
        ...
        logger.error(
            f"Could not transform given layers {layers} into a valid format")
        return "layerstring_malformed"

    if min(layer_widths) < 1:
        logger.error(
            f"received layer with invalid width. Please choose higher width")
        return "too_small_width_detected"
    if min(layer_widths) < 5:
        logger.warning(
            f"Using very thin layers is typically not that useful. received hidden layer with {min(layer_widths)} Units")
    if max(layer_widths) > 2000:
        logger.error(
            f"Detected layer of unreasonable width: {max(layer_widths)}. Using thiner, yet a longer cascade of layers is probably better suited")
...
def valid_conv_and_pool_layers(conv_and_pool_layers: list):
    ...
    for layer in conv_and_pool_layers:
        ...
        logger.error(
            f"received layer which is neither a convolution nor a poolng layer: {layer}")
...
def valid_pooling_layer(layer: str):
    ...
    if len(pooling_str_splitted) != 4:
        logger.error(
            f"Invalid format for convolution layer given. Please use format <pooling-method>_pool_<kernel-size>_<stride> \n Got: {layer}")
    ...
    try:
        ...
        logger.error(
            f"Invalid format for pooling layer given. Please use format <pooling-method>_pool_<kernel-size>_<stride> \n Got: {layer}")
        return "got_malformed_pool_str"

    valid_pooling_methods = ["max", "mean"]
    if pooling_method not in valid_pooling_methods:
        logger.error(
            f"received invalid pooling method {pooling_method}. Please choose from {valid_pooling_methods}")
...
def valid_convolution_layer(layer: str):
    ...
    if len(convolution_string_splitted) != 4:
        logger.error(
            f"Invalid format for convolution layer given. Please use format conv_<nr_filters>_<kernel_size>_<stride> \n Got: {layer}")
...
def valid_amount_of_convolution_filters(nr_filters: int):
    ...
    if nr_filters < 1:
        logger.error(
            f"received convolution layer with too small amount of filters : {nr_filters}")
        return "received_too_small_amount_of_filters"
    if nr_filters > 50:
        logger.warning(
            f"Used suspiciously large amount of filters: {nr_filters}")
    if nr_filters > 100:
        logger.error(
            f"Amount of given filters for convolution layer is too big: {nr_filters}")
...
def split_layerstring_into_conv_and_ff_layers(layerstring: str):
    ...
    for layer in layerstring_splitted:
        ...
        try:
            ...
            logger.error(
                f"Could not interpret layer {layer}.  Please use format conv_<nr_filters>_<kernel_size>_<stride>")
            return [], [], f"could_not_interpret_layer_{layer}"
        ff_layers.append(layer)

    if conv_and_pool_layers == []:
        logger.error(
            f"received CNN Model without any convolution or pooling layer specified")
...
def build_layers_from_string(layerstring: str, network_type: str):
    ...
    logger.error(
        f"Calculation of the number and structure of other network types implemented yet")
...
def set_optimizer(optimizer: str, learning_rate: float, model: nn.Module):
    ...
    if (learning_rate > 5) or (learning_rate < 0):
        logger.error(
            f"received invalid learning rate if {learning_rate}. Please choose in [0,5]")
        return None, "learning_rate_out_of_allowed_boundaries"

    if learning_rate > 1:
        logger.warning(
            f"Choosing a high learning rate might prevent the model from learning at all. A learning rate [0,1] is recommended. Got: {learning_rate}")
...
def save_model(model: nn.Module, saving_folder: str, model_name = "model.pt"):
    ...
    if not os.path.exists(full_model_path):
        logger.error(
            f"Could not find saved model file in location {full_model_path}")
    ...
    with torch.no_grad():
        for j, minibatch in enumerate(loader):
            ...
            if j == 0:
                ...
                y_truth = torch.cat(
                    (y_truth, bring_to_correct_format(minibatch[1])), 0)

        # Create the final confusion matrix of the data
        conf_mat = ConfusionMatrix(len(torch.unique(y_truth)))
        ...
        if verbose:
            logger.info(
                f"This is the Confusion Matrix of the {data_set}-data after Training: (n = {torch.sum(confusion_matrix).item()})\n{confusion_matrix}")
        logger.info(
            f"The final accuracy on {round(frac*100, 2)}% of the {data_set} data is {accuracy}%")

        plot_utils.plot_confusion_matrix(
            confusion_matrix, store_local, saving_folder, data_set, hierarchical_model)
...
def update_validation_accuracies_and_losses(validation_losses: list, validation_accuracies: list, validationloader, criterion, model: nn.Module, frac=1.0):
    ...
    with torch.no_grad():
        ...
        for j, validation_minibatch in enumerate(validationloader):
            ...
            y_pred_validation = validation_model.predict(
                validation_minibatch[0].double())
            # transform the ground truth for this batch into an appropriate format for the loss function
            y_truth_validation = bring_to_correct_format(
                validation_minibatch[1])

            loss_validation_minbatch = validation_criterion(
                y_pred_validation, y_truth_validation)
            ...
            y_pred_validation_transformed = transform_predictions_for_confusion_matrix(
                y_pred_validation)

            accuracy_batch = sum_identical_entries(
                y_truth_validation, y_pred_validation_transformed).item() / len(validation_minibatch[1])
            validation_accuracies_epoch.append(accuracy_batch)

    average_validation_data_loss = sum(
        validation_losses_epoch)/len(validation_losses_epoch)
    validation_losses.append(average_validation_data_loss) if isinstance(
        average_validation_data_loss, float) else validation_losses.append(average_validation_data_loss.item())

    average_validation_accuracy = sum(
        validation_accuracies_epoch)/(len(validation_accuracies_epoch))
...
def create_ffnn_model(batch_normalization: bool, seed: int):
    ...
    class FeedforwardNetwork(nn.Module):
        def __init__(self, n_features: int, out_sz: int, layers: list, dropout_rate: float, batch_normalization: bool):
            ...
            # Create a list of layers with batchnormalization and dropout, if specified
            layerlist = build_fully_connected_layers(
                n_features, [int(x) for x in layers], batch_normalization, dropout_rate, out_sz)
...
def create_convolutional_model(batch_normalization: bool, seed: int):
    ...
    class ConvolutionalNetwork(nn.Module):
        def __init__(self, n_features: int, out_sz: int, layers: list, dropout_rate: float, batch_normalization: bool):
            ...
            if error:
                logger.error(
                    f"Could not build convolution and pooling layers, adjusting the checks in valid_table() is needed")
                return
            ...
            fully_connected_layers = build_fully_connected_layers(
                n_features, remaining_layers, batch_normalization, dropout_rate, out_sz)
...
def build_convolution_layer(n_in: int, convolution_string: str):
    ...
    if "conv" not in convolution_string:
        logger.error(
            f"received malformed convolution_string. Please use format conv_<nr_filters>_<kernel_size>_<stride>")
    ...
    try:
        ...
        logger.error(
            f"Extraction of needed Information from convolution_string failed. Please use format conv_<nr_filters>_<kernel_size>_<stride> . Got {convolution_string}")
...
def build_pooling_layer(pooling_str: str):
    ...
    if pooling_str.count("_") != 3:
        logger.error(
            f"received malformed pooling_str. Please use format  <pooling-method>_pool_<kernel_size>_<stride>")
        return None, f"received_malformed_pooling_str_{pooling_str}"
    ...
    try:
        ...
        logger.error(
            f"Extraction of needed Information from pooling_str failed. Please use format  <pooling-method>_pool_<kernel_size>_<stride> . Got {pooling_str}")
        return None, f"information_extraction_failed_from_pooling_str_{pooling_str}"

    valid_pooling_methods = ["max", "mean"]
    if pooling_method not in valid_pooling_methods:
        logger.error(
            f"received invalid pooling method {pooling_method}. Choose from {valid_pooling_methods}")
...
def Dataset_from_local_drive(folder: str, hierarchical_model: str):
    ...
    class CustomDataset(TensorDataset):
        def __init__(self, folder):
            ...
            all_files, error = if_hierarchical_data_remove_unneeded_files(
                all_files, folder, hierarchical_model)
            if error:
                return
            ...
            labels_df["label"] = labels_df["filename"].astype(str).str.split(
                "/").apply(lambda x: x[-1]).astype(str).str.split("_").apply(lambda x: x[0]).apply(lambda x: x[-1]).astype(int)

            self.labels = labels_df
            self.folder = folder

            hierarchical_data_seen, error = processing_featured_hierarchical_data(
                folder)
            if error:
                return

            if hierarchical_data_seen:
                svm_model_folder, error = find_correct_svm_folder(
                    folder, hierarchical_model)
                if error:
                    return

                standardizing_df, standardization_type, error = data_utils.prepare_standardization(
                    svm_model_folder, True, hierarchical_model)
                if error:
                    return
            ...
            self.data_in_channel_format = log_file.get(
                "prepare_dataset").get("convolutional")
...
def Dataset_from_RAM(folder: str, epochs_to_train: float, hierarchical_model: str):
    ...
    if hierarchical_data_seen:
        joined_df, error = standardize_before_concatting(
            result, folder, hierarchical_model)
        if error:
            return None, error
    else:
    ...
    if x.shape[0] != y.shape[0]:
        nr_channels = len(pd.read_csv(all_files[0], usecols=[
                          0], dtype=np.float64, header=None))
...
def return_dataset(folder: str, data_set: str, epochs: float, hierarchical_model: str):
    ...
    available_RAM_in_GB, needed_ram, error = check_needed_and_available_ram(
        folder, hierarchical_model, epochs, data_set)
    if error:
        return None, error
    ...
    if (available_RAM_in_GB - needed_ram*4) > 2:
        logger.info(
            f"As reading in the {data_set}-dataset into is RAM possible, therefore starting to read-in files")
...
def create_loader(folder: str, minibatch_size: int, data_set: str, epochs: float, hierarchical_model: str):
    """returns a Dataloader object which returns the data batchwise ith the given minibatch_size
    """
    dataset, error = return_dataset(
        folder, data_set, epochs, hierarchical_model)
    if error:
        return None, error
...
def get_n_features(folder: str, hierarchical_model: str):
    ...
    featured_hierarchical_data, error = processing_featured_hierarchical_data(
        folder)
    if error:
        return 0, error
    ...
    if len(all_files) < 2:
        logger.error(
            f"Given folder does not contain the minimum amount of files: {folder}")
        return 0, "folder_does_not_contain_enough_files"

    # Pick any file which is not the labels.csv file
    suitable_file = all_files[0] if not all_files[0].endswith(
        "labels.csv") else all_files[1]
...
def get_out_szs(folder: str, hierarchical_model: str):
    ...
    if featured_hierarchical_data:
        out_szs = correct_out_szs_for_hierarchical_data(
            df["label"], hierarchical_model)
...
def valid_first_width_after_flattening(folder: str, conv_and_pool_layers: list, width_of_first_layer_after_flattening: int, padding=0):
    ...
    nr_observations = len(pd.read_csv(
        all_files[0], nrows=1, header=None, dtype=np.float64).columns)
    all_filters = [1]
    ...
    for _, layer in enumerate(conv_and_pool_layers, start=1):
        ...
        logger.error(
            f"received layer which is neither convolution or polling layer : {layer}")
    ...
    if nr_flattened_units != int(width_of_first_layer_after_flattening):
        logger.error(
            f"To contain the flattened data after convolution and pooling {nr_flattened_units} units are needed, but {width_of_first_layer_after_flattening} were given")
...
def check_needed_and_available_ram(folder: str, hierarchical_model: str, epochs: float, data_set: str):
    ...
    available_RAM_in_GB = round(
        psutil.virtual_memory().available/1000000000, 2)
    files_in_dir, error = file_utils.get_files(folder, "csv")
    if error:
        return None, None, error

    files_in_dir, error = if_hierarchical_data_remove_unneeded_files(
        files_in_dir, folder, hierarchical_model)
    if error:
        return 0., 0., error
    ...
    logger.info(
        f"The needed RAM to read-in the (part) of {data_set}-dataset is {needed_ram} GB, while {available_RAM_in_GB} are available")
...
def reduce_to_relevant_files_for_hierarchical_model(list_of_files: list, hierarchical_model: str):
    ...
    if hierarchical_model not in valid_hierarchical_levels:
        logger.error(
            f"received invalid level {hierarchical_model} Choose from {valid_hierarchical_levels}")
...
def if_hierarchical_data_remove_unneeded_files(list_of_files: list, folder: str, hierarchical_model: str):
    ...
    if featured_hierarchical_data:
        list_of_files, error = reduce_to_relevant_files_for_hierarchical_model(
            list_of_files, hierarchical_model)
        if error:
...
def processing_featured_hierarchical_data(folder: str):
    ...
    if "balancing_over" not in log_file.keys():
        logger.error(
            f"found log file which does not contain information regarding oversampling")
        return True, "log_file_does_not_contain_information_regarding_oversampling"

    return "generate_features" in log_file.keys() and log_file["balancing_over"]["label_depth"] == 2, None
...
def create_rnn_model(batch_normalization: bool, seed: int, log_file={}):
    ...
    class RecurrentNetwork(nn.Module):
        def __init__(self, n_features: int, out_sz: int, layers: list, dropout_rate: float, batch_normalization: bool):
            ...
            self.nr_timestamps_seen = int(log_file.get("create_windows").get(
                "window_length") / log_file.get("create_windows").get("resampling_rate"))
...
def valid_hidden_size(layerstring: str):
    ...
    try:
        ...
        logger.error(
            f" Cannot convert given layerstring {layerstring} to a valid int")
...
def load_needed_params(model_folder: str, network_type: str):
    ...
    batch_normalization = model_log_file.get(
        "train_model").get("batch_normalization")
    if not isinstance(batch_normalization, bool):
        logger.error(
            f"The read-in batch_normalization value from log is not a bool")
    ...
    if not isinstance(dropout_rate, float):
        logger.error(
            f"The read-in dropout_rate value from log is not an float")
        return None, None, None, None, None, None, "received_non_float_dropout_rate"

    layers, error = build_layers_from_string(model_log_file.get(
        "train_model").get("layer_structure"), network_type)
    if error:
...
def load_model(model_folder: str, network_type: str, log_file: dict, model_name = "model.pt"):
    ...
    seed, batch_normalization, n_features, out_szs, dropout_rate, layers, error = load_needed_params(
        model_folder, network_type)
    if error:
        return None, error

    Model = build_nn_model(network_type, batch_normalization, seed, log_file)
    model = Model(n_features, out_szs, layers,
                  dropout_rate, batch_normalization)
    model.load_state_dict(torch.load(os.path.join(
        model_folder, model_name)), strict=False)
...
def evaluate_on_validation_data(model: nn.Module, saving_folder: str, validation_losses: list, validation_accuracies: list, validationloader: DataLoader, criterion, frac: float, keep_calc: bool, time_unit: str, time_nr: int, network_type: str, log_file: dict):
    ...
    validation_accuracies, validation_losses = update_validation_accuracies_and_losses(
        validation_losses, validation_accuracies, validationloader, criterion, model, frac)
    logger.info(
        f'The accuracy on {round(frac*100,2)}% of the validation data after {time_unit} {time_nr} is {round(validation_accuracies[-1]*100,2)}%')
...
def find_correct_svm_folder(folder: str, hierarchical_model: str):
    """detects the respective svm folder with the standarization file
    """
    log_position = os.path.join(folder, "log.json")
    if not os.path.exists(log_position):
        logger.error(
            f"Could not find a log file in the currently handeled folder {folder}")
