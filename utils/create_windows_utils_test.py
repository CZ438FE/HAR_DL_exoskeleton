...
class TestCreateWindowsUtils(unittest.TestCase):
    def test_prepare_time_col(self):
        # test if the wrong datatype was given
        got_df, got_error = create_windows_utils.prepare_time_col(
            [], "/somewhere/some_file.csv")
        ...
        got_df, got_error = create_windows_utils.prepare_time_col(
            pd.DataFrame({"some_col": [1, 2, 3]}), "/somewhere/some_file.csv")
        assert_frame_equal(got_df, pd.DataFrame())
        ...
        got_df, got_error = create_windows_utils.prepare_time_col(pd.DataFrame(
            {"time": [[1, 15], [1854, 484], [546851, 456]], "some_col": [1, 2, 3]}), "/somewhere/some_file.csv")
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_error, 'got_col_time_of_nonint_dt')

        # Test if the given time col cannot be brought to the same len
        got_df, got_error = create_windows_utils.prepare_time_col(pd.DataFrame(
            {"time": [1669279860, 0, 1669279868], "some_col": [1, 2, 3]}), "/somewhere/some_file.csv")
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_error, "cannot_bring_0_to_maxlen")

        # Test if the seen time begins too early
        got_df, got_error = create_windows_utils.prepare_time_col(pd.DataFrame(
            {"time": [1669279860000, 1000000000000, 1669279868000], "some_col": [1, 2, 3]}), "/somewhere/some_file.csv")
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(
            got_error, "earliest_date_seen_from_year_smaller_2018")

        # Test if the seen time ends too late
        got_df, got_error = create_windows_utils.prepare_time_col(pd.DataFrame(
            {"time": [2041408633505, 2041408633506, 2041408633507], "some_col": [1, 2, 3]}), "/somewhere/some_file.csv")
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_error, "latest_date_seen_from_year_greater_2023")

        # Test if the seen time duration is too long
        got_df, got_error = create_windows_utils.prepare_time_col(pd.DataFrame(
            {"time": [1669279860000, 1669309861000, 1669279868000], "some_col": [1, 2, 3]}), "/somewhere/some_file.csv")
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_error, "seen_duration_longer_eight_hours")

        # Test if everything works as intended
        got_df, got_error = create_windows_utils.prepare_time_col(pd.DataFrame(
            {"time": [166927986000, 1669279861000, 1669279865000], "some_col": [1, 2, 3]}), "/somewhere/some_file.csv")
        want_df = pd.DataFrame(
            {"time": [1669279860000, 1669279861000, 1669279865000], "some_col": [1, 2, 3]})
        assert_frame_equal(got_df, want_df)
        self.assertIsNone(got_error)

    def test_create_list_of_windows(self):
        # test if there is no time col in the given df
        df = pd.DataFrame({"another_col": [
                          1669279860000, 1669279860010, 1669279860015], "some_col": [1, 2, 3]})
        got_list, got_error = create_windows_utils.create_list_of_windows(
            df, 1000, 500, "some_file")
        self.assertListEqual(got_list, [])
        self.assertEqual(got_error, "no_time_col_found")

        # test if the window_length is greater than the observed session duration
        df = pd.DataFrame(
            {"time": [1669279860000, 1669279860010, 1669279860015], "some_col": [1, 2, 3]})
        got_list, got_error = create_windows_utils.create_list_of_windows(
            df, 1000, 500, "some_file")
        self.assertListEqual(got_list, [])
        self.assertIsNone(got_error)
        ...
        # test if everything works as intended
        df = pd.DataFrame(
            {"time": [1669279860000, 1669279864000], "some_col": [1, 2]})
        got_list, got_error = create_windows_utils.create_list_of_windows(
            df, 1000, 1000, "some_file")
        self.assertListEqual(got_list, [[1669279860000, 1669279861000], [1669279861000, 1669279862000], [
                             1669279862000, 1669279863000], [1669279863000, 1669279864000]])
        self.assertIsNone(got_error)

    def test_handle_window(self):
        valid_df = pd.DataFrame({"time": [x for x in range(1669279860000, 1669279870000, 1000)], "some_col": [
                                x for x in range(0, 10000, 1000)], "label": [11]*10})
        no_time_col_df = pd.DataFrame({"another_col": [x for x in range(
            1669279860000, 1669279870000, 1000)], "some_col": [x for x in range(0, 10000, 1000)], "label": [11]*10})
        only_nan_df = pd.DataFrame({"time": [x for x in range(
            1669279860000, 1669279870000, 1000)], "some_col": [np.nan]*10, "label": [11]*10})
        no_label_col_df = pd.DataFrame({"time": [x for x in range(
            1669279860000, 1669279870000, 1000)], "some_col": [x for x in range(0, 10000, 1000)]})

        data_columns = ["some_col"]
        valid_window = [1669279860000, 1669279861000]
        valid_saving_folder = "/tmp/trainings_pipeline/create_windows_utils_test/test_handle_windows"

        # Test if the df has the wrong datatype
        got_success, got_windows_saved, got_error = create_windows_utils.handle_window(
            [], valid_window, valid_df, data_columns, True, "some_file.csv", 1000, 10, True, valid_saving_folder, 0)
        self.assertIsNone(got_success)
        self.assertEqual(got_windows_saved, 0)
        self.assertEqual(got_error, "given_df_no_pdDataFrame")

        # test if the df_filled has wrong dt
        got_success, got_windows_saved, got_error = create_windows_utils.handle_window(
            valid_df, valid_window, [], data_columns, True, "some_file.csv", 1000, 10, True, valid_saving_folder, 0)
        self.assertIsNone(got_success)
        self.assertEqual(got_windows_saved, 0)
        self.assertEqual(got_error, "given_df_filled_no_pdDataFrame")
        ...
        self.assertIsNone(got_success)
        self.assertEqual(got_windows_saved, 0)
        self.assertEqual(got_error, "no_label_col_found")
        ...
        got_success, got_windows_saved, got_error = create_windows_utils.handle_window(no_time_col_df, [
                                                                                       "sdf", "fdh"], valid_df, data_columns, True, "some_file.csv", 1000, 10, True, valid_saving_folder, 0)
        self.assertIsNone(got_success)
        self.assertEqual(got_windows_saved, 0)
        self.assertEqual(got_error, "no_time_col_found")
        ...
        got_success, got_windows_saved, got_error = create_windows_utils.handle_window(
            only_nan_df, valid_window,  valid_df, data_columns, True, "some_file.csv", 1000, 10, True, valid_saving_folder, 0)
        self.assertFalse(got_success)
        self.assertEqual(got_windows_saved, 0)
        self.assertIsNone(got_error)

        # Test if the correct result gets returned without saving and without flatten
        got_success, got_windows_saved, got_error = create_windows_utils.handle_window(
            valid_df, valid_window, valid_df, data_columns, False, "some_file.csv", 1000, 10, False, valid_saving_folder, 0)
        self.assertTrue(got_success)
        self.assertEqual(got_windows_saved, 0)
        self.assertIsNone(got_error)
        ...
        self.assertTrue(got_success)
        self.assertEqual(got_windows_saved, 1)
        self.assertIsNone(got_error)
        exists = os.path.exists(os.path.join(
            valid_saving_folder, "some_file_1.csv"))
        self.assertTrue(exists)
        saved_df = pd.read_csv(os.path.join(
            valid_saving_folder, "some_file_1.csv"))
        want_df_not_flattened = pd.DataFrame(
            {"time": [1669279860000, 1669279861000], "some_col": [0, 1000], "label": [11, 11]})
    ...
    def test_create_windows_for_file(self):
        # Test if df does not contain the needed columns
        foldername = "/tmp/trainings_pipeline/create_windows_utils_test/test_create_windows_for_file"
        ...
        want = data_utils.create_result_string(
            path_to_file, [1], 0, "earliest_date_seen_from_year_smaller_2018")
        self.assertEqual(got, want)

        # Test if an unknown filling method is given
        df = pd.DataFrame(
            {"time": [1669305215000, 1669307215000], "some_col": [12, 11]})
        df.to_csv(path_to_file, index=False)
        ...
        got = create_windows_utils.create_windows_for_file(
            file_with_attributes)
        want = data_utils.create_result_string(
            path_to_file, [1], 0, "invalid_filling_method_unknown_filling_method_given")
        self.assertEqual(got, want)

        # Test if everything works as intended
        df = pd.DataFrame({"time": [1669305215000, 1669305216000], "some_col": [
                          12, 11], "label": [11, 11]})
        df.to_csv(path_to_file, index=False)
    ...
    def test_valid_attributes(self):
        ...
        saving_folder_valid = "/tmp/trainings_pipeline/create_windows_utils_test/test_valid_attributes"
        ...
        self.assertEqual(got, "given_labeled_files_not_a_list")
        ...
        self.assertEqual(got, "got_file_of_nonstr_type")
        ...
        got = create_windows_utils.valid_attributes(needed_columns_empty, needed_columns_valid, window_length_valid, step_length_valid,
                                                    filling_method_valid, resampling_rate_valid, data_columns_valid, flatten_valid, store_local_valid, saving_folder_valid, None)
        self.assertEqual(got, "labeled_files_empty")
        ...
        got = create_windows_utils.valid_attributes(labeled_files_valid, 5, window_length_valid, step_length_valid, filling_method_valid,
                                                    resampling_rate_valid, data_columns_valid, flatten_valid, store_local_valid, saving_folder_valid, None)
        self.assertEqual(got, "needed_columns_not_a_list")
        ...
        got = create_windows_utils.valid_attributes(labeled_files_valid, [], window_length_valid, step_length_valid, filling_method_valid,
                                                    resampling_rate_valid, data_columns_valid, flatten_valid, store_local_valid, saving_folder_valid, None)
        self.assertEqual(got, "needed_columns_empty")

        # test wrong window_length
        got = create_windows_utils.valid_attributes(labeled_files_valid, needed_columns_valid, [
        ], step_length_valid, filling_method_valid, resampling_rate_valid, data_columns_valid, flatten_valid, store_local_valid, saving_folder_valid, None)
        self.assertEqual(got, "non_int_window_length")
        ...
        got = create_windows_utils.valid_attributes(labeled_files_valid, needed_columns_valid, window_length_invalid, step_length_valid,
                                                    filling_method_valid, resampling_rate_valid, data_columns_valid, flatten_valid, store_local_valid, saving_folder_valid, None)
        self.assertEqual(got, "window_length_too_small")
        ...
        got = create_windows_utils.valid_attributes(labeled_files_valid, needed_columns_valid, window_length_valid, window_length_invalid,
                                                    filling_method_valid, resampling_rate_valid, data_columns_valid, flatten_valid, store_local_valid, saving_folder_valid, None)
        self.assertEqual(got, "step_length_too_small")

        # Test spep length > window_length
        got = create_windows_utils.valid_attributes(labeled_files_valid, needed_columns_valid, window_length_valid, window_length_valid +
                                                    200, filling_method_valid, resampling_rate_valid, data_columns_valid, flatten_valid, store_local_valid, saving_folder_valid, None)
        self.assertEqual(got, "step_length_bigger_window_length")
        ...
        self.assertEqual(got, "filling_method_not_implemented")

        # wrong dtype resampling_rate
        got = create_windows_utils.valid_attributes(labeled_files_valid, needed_columns_valid, window_length_valid, window_length_valid, filling_method_valid, [
                                                    resampling_rate_valid], data_columns_valid, flatten_valid, store_local_valid, saving_folder_valid, None)
        self.assertEqual(got, "got_resampling_rate_of_nonint_type")
        ...
        self.assertEqual(got, "resampling_rate_too_small")
        ...
        self.assertEqual(got, "resampling_rate_too_big")
        ...
        self.assertEqual(got, "data_col_not_subset_of_needed_cols")

        # wrong datatype for flatten
        got = create_windows_utils.valid_attributes(labeled_files_valid, needed_columns_valid, window_length_valid, window_length_valid,
                                                    filling_method_valid, resampling_rate_valid, data_columns_valid, [flatten_valid], store_local_valid, saving_folder_valid, None)
        self.assertEqual(got, "got_flatten_of_nonbool_type")

        # wrong datatype for store_local
        got = create_windows_utils.valid_attributes(labeled_files_valid, needed_columns_valid, window_length_valid, window_length_valid,
                                                    filling_method_valid, resampling_rate_valid, data_columns_valid, flatten_valid, [store_local_valid], saving_folder_valid, None)
        self.assertEqual(got, "got_store_local_of_nonbool_type")

        # wrong datatype saving_folder
        got = create_windows_utils.valid_attributes(labeled_files_valid, needed_columns_valid, window_length_valid, window_length_valid,
                                                    filling_method_valid, resampling_rate_valid, data_columns_valid, flatten_valid, store_local_valid, [saving_folder_valid], None)
        self.assertEqual(got, "got_saving_folder_of_nonstr_type")
        ...
        self.assertIsNone(create_windows_utils.valid_attributes(labeled_files_valid, needed_columns_valid, window_length_valid, window_length_valid,
                          filling_method_valid, resampling_rate_valid, data_columns_valid, flatten_valid, store_local_valid, saving_folder_valid, None))
        self.assertTrue(os.path.exists(saving_folder_valid))

    def test_create_list_of_file_dicts(self):
        labeled_files_valid = [
            "/somewhere/somefile.csv", "/somewhere/another_file.csv"]
        needed_columns_valid = ["time", "label", "some_col"]
        window_length_valid = 1000
        window_length_invalid = -1000
        step_length_valid = 100
        ...
        self.assertListEqual(got_list, [])
        self.assertEqual(got_error, "non_int_window_length")
    ...
    def test_valid_table(self):
        existing_datapath = "/tmp/trainings_pipeline/create_windows_utils_test/test_valid_table/"
        ...
        self.assertEqual(create_windows_utils.valid_table(
            table), "dp_does_not_exist")
        ...
        error = file_utils.save_json(valid_anonymize_file, os.path.join(
            existing_datapath, "anonymization_file.json"))
        self.assertIsNone(error)

        self.assertEqual(create_windows_utils.valid_table(
            table), "dir_labeled_data_not_subfolder_of_dp")
        ...
        self.assertEqual(create_windows_utils.valid_table(
            table), "got_dryrun_of_nonbool_datatype")

        # Test if the given window_length or step_length are nor valid
        table = {"data_path": existing_datapath, "dryrun": True,
                 "window_length": -20, "step_length": 200000, }
        self.assertEqual(create_windows_utils.valid_table(
            table), "given_window_length_smaller_1")

        # Test if the given resamling_rate is not valid
        table = {"data_path": existing_datapath, "dryrun": True, "window_length": 1000,
                 "step_length": 500, "resampling_rate": "wrong_datatype"}
        self.assertEqual(create_windows_utils.valid_table(
            table), "got_resampling_rate_of_nonint_dataytpe")

        # Test if the given resampling_rate is not valid
        table = {"data_path": existing_datapath, "dryrun": True,
                 "window_length": 1000, "step_length": 500, "resampling_rate": -200}
        self.assertEqual(create_windows_utils.valid_table(
            table), "resampling_rate_too_small")

        # Test if the given flatten value is not valid
        table = {"data_path": existing_datapath, "dryrun": True, "window_length": 1000,
                 "step_length": 500, "resampling_rate": 10, "flatten": "not_a_bool"}
        self.assertEqual(create_windows_utils.valid_table(
            table), "got_flatten_of_nonbool_type")
        ...
        self.assertEqual(create_windows_utils.valid_table(
            table), "got_time_str_of_invalid_len")

        # Test if there are no files in the given folder
        table = {"data_path": existing_datapath, "dryrun": True, "window_length": 1000,
                 "step_length": 500, "resampling_rate": 10, "flatten": True}
        self.assertEqual(create_windows_utils.valid_table(
            table), "no_data_in_dir")

        # test when everything is valid
        # Create a file in the labeled_data subfolder
        df = pd.DataFrame({"some_col": [1, 2, 3]})
        df.to_csv(os.path.join(existing_datapath,
                  "labeled_data", "somefile.csv"), index=False)

        table = {"data_path": existing_datapath, "dryrun": True, "window_length": 1000,
                 "step_length": 500, "resampling_rate": 10, "flatten": True}
        self.assertIsNone(create_windows_utils.valid_table(table))

    def test_return_normalization_df(self):
        ...
        anonymize_file = {"create_windows": {"data_columns": ["some_col", "another_col"],
                                             ...
                                             "z_normalize_std": [2, 3]}}
        got_df, got_error = create_windows_utils.return_normalization_df(
            "z_normalize", anonymize_file)
        self.assertIsNone(got_error)
