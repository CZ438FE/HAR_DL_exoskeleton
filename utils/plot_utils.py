...
def return_dict_nr_to_label(level: str):
    ...
    if level not in valid_levels:
        logger.error(
            f"Invalid granularity given. Choose one of {valid_levels} ")
        return {}, "invalid_granularity_given"
    if level == 'top':
        return {"1": "Lifting", "2": "Walking", "3": "Resting", "4": "Driving"}, None
    if level == 'mid':
        return {"11": "Lifting", "12": "Dropping", "13": "Pushing", "14": "Pulling", "15": "Holding", "21": "Walking Straight", "22": "Walking Upstairs", "23": "Walking Downstairs",
                "24": "Walking Sidesteps", "31": "Resting", "32": "Sitting", "33": "Lying", "41": "Driving Standing", "42": "Driving Sitting"}, None
    return {"11": "Lifting", "12": "Dropping", "13": "Pushing", "14": "Pulling", "15": "Holding", "211": "Walking Free", "212": "Carrying", "213": "Walking Pushing",
            ...
            "31": "Standing", "32": "Sitting", "33": "Lying", "41": "Driving Standing", "42": "Driving Sitting"}, None
...
def change_label_nr_to_human_readable(label_dict: dict, dict_nr_to_label: dict):
    ...
    if not dict_nr_to_label:
        logger.error(
            "Cannot change the labels when the dict containing the mapping information is empty")
        return {}
...
def show_label_distribution(granularity: str, labels_distribution: dict, saving_folder: str, store_local=False):
    ...
    human_readable_dict = change_label_nr_to_human_readable(
        labels_distribution, nr_to_label_dict)
...
def infer_level(label: list):
    ...
    if isinstance(label, str):
        ...
        logger.error(
            f"Got invalid datatype for inferring the label {type(label)}")
...
def replace_list_labels_with_human_readable_form(list_of_labels: list):
    ...
    human_readable_levels = [dict_nr_to_label.get(
        str(label)) for label in list_of_labels]
...
def plot_original_samples_on_data_after_resampling(prop_original_data_dict: dict, saving_folder: str, store_local: bool):
    ...
    if not set(["class", "prop_existing_after_balancing"]).issubset(set(prop_original_data_dict.keys())):
        logger.error(
            f"Got malformed dict with keys {prop_original_data_dict.keys()}")
    ...
    internal_prop_original_data_dict["class"], error = replace_list_labels_with_human_readable_form(
        internal_prop_original_data_dict.get("class"))
    if error:
        return error
...
def plot_accuracies_and_loss_over_batches(train_accuracies: list, val_accuracies: list, trainings_losses: list, val_losses: list, store_local: bool, saving_folder: str, validation_data_step_length: int):
    ...
    if train_accuracies != []:
        plotted_something = plot_course(
            train_accuracies, "minibatch", "Accuracy", "Training", store_local, saving_folder)
    else:
        logger.warning(
            f"Could not plot the change of accuracies, as the list of train_accuracies was empty")
    if trainings_losses != []:
        plotted_something = plot_course(
            trainings_losses, "minibatch", "Loss", "Training", store_local, saving_folder)
    else:
        logger.warning(
            f"Could not plot the change of losses, as the list of train_losses was empty")

    if val_accuracies != []:
        plotted_something = plot_course(
            val_accuracies, f"minibatch (in {validation_data_step_length})", "Accuracy", "Validation", store_local, saving_folder)
    if val_losses != []:
        plotted_something = plot_course(
            val_losses, f"minibatch (in {validation_data_step_length})", "Loss", "Validation", store_local, saving_folder)

    # Inform the user that the plots were stored
    if store_local and plotted_something:
        logger.info(
            f"Saved the plot(s) in the speciefied saving folder {saving_folder}")
...
def get_class_names_from_len(len_df: int, hierarchical_model: str):
    ...
    if len_df == 3 and hierarchical_model == "top":
        ...
        return ["lifting", "dropping", "holding"]
    elif len_df == 3 and hierarchical_model == "walking":
        return ["walking\nStraight", "Walking\nUpstairs", "Walking\nDownstairs"]
    return ["lifting", "dropping", "holding", "walking\nstraight", "walking\nupstairs", "walking\ndownstairs", "resting"]
...
def plot_confusion_matrix(confusion_matrix, store_local: bool, saving_folder: str, data_set: str, hierarchical_model: str):
    ...
    plt.title(f"Confusion Matrix of the {data_set} data")
    if store_local:
        plt.savefig(os.path.join(saving_folder,
                    f"{data_set}_confusion_matrix.png"))
    plt.show()
