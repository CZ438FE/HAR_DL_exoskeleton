...
class TestDataUtils(unittest.TestCase):
    def test_has_identical_length(self):
    ...
    def test_only_nan(self):
        ...
        self.assertIsNone(data_utils.only_nan(
            test_df, ["core"]), "contains_only_nan")

        # Test if nan in col
        test_df = pd.DataFrame(
            {"numbers": [1, 23, 53], "core": [np.nan, np.nan, np.nan]})
        self.assertEqual(data_utils.only_nan(
            test_df, ["core"]), "contains_only_nan")

        # Test if mixed values col
        test_df = pd.DataFrame(
            {"numbers": [1, 23, 53], "core": [12, np.nan, 15]})
    ...
    def test_valid_window_and_step_length(self):
        ...
        self.assertEqual(data_utils.valid_window_and_step_length(
            None, 200), "window_length_is_None")

        # test when  no step length is given
        self.assertEqual(data_utils.valid_window_and_step_length(
            1000, None), "step_length_is_None")

        # test when the window length is too small
        self.assertEqual(data_utils.valid_window_and_step_length(-250,
                         200), "given_window_length_smaller_1")

        # test when the step length is too small
        self.assertEqual(data_utils.valid_window_and_step_length(
            2050, -35), "given_step_length_smaller_1")

        # test when the step length is greater than the window length
        self.assertEqual(data_utils.valid_window_and_step_length(
            100, 100000), "step_length_greater_window_length")
    ...
    def test_create_filled_df(self):
        ...
        got_df, got_error = data_utils.create_filled_df(
            valid_df, "an_invalid_filling_method")
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(
            got_error, "invalid_filling_method_an_invalid_filling_method_given")
        ...
        valid_df = pd.DataFrame({"time": [1000, 1009], "label": [
                                12, 12], "some_col": [10, 19]})
    ...
    def test_flatten(self):
        ...
        invalid_df = pd.DataFrame(
            {"time": [11, 11], "some_col": [12, 13], "label": [12, 12]})
        got_df, got_error = data_utils.flatten(invalid_df, "some_file.csv")
    ...
    def test_find_first_index_bigger_x_in_sorted_list(self):
        ...
        value = 8
        got = data_utils.find_first_index_bigger_x_in_sorted_list(
            example_list, value)
    ...
    def test_split_files_to_handle_into_chunks(self):
        ...
        want = [[x for x in range(10)], [x for x in range(10, 20)], [x for x in range(
            20, 30)], [x for x in range(30, 40)], [x for x in range(40, 50)]]
        self.assertListEqual(got, want)
    ...
    def test_create_result_string(self):
        ...
        file = "somefile.csv"
        got = data_utils.create_result_string(
            file, [], 0, "i_should_not_appear", )
    ...
    def test_downsize_label(self):
        ...
        got_list, got_error = data_utils.downsize_label(
        ...
        label_depth = 3
        got_list, got_error = data_utils.downsize_label(
            valid_label_series, label_depth)
        self.assertIsNone(got_list)
        self.assertEqual(
            got_error, "granularity_of_label_col_smaller_than_label_depth")
    ...
    def test_label_class_mean(self):
        ...
        test_df = pd.DataFrame({"only_na": [np.nan, np.nan, np.nan], "one_NA": [
                               1, 1, np.nan], "label": [1, 1, 1]})
        got_dict, got_error = data_utils.label_class_mean(
            test_df, "only_na", "label")
        self.assertEqual(
            got_error, "label_class_mean_cannot_be_calculated_for_row_with_only_NAN")
        self.assertDictEqual(got_dict, {})

        # test if the df does not contain a label col
        test_df = pd.DataFrame({"only_na": [np.nan, np.nan, np.nan], "one_NA": [
                               1, 1, np.nan], "some_col": [1, 1, 1]})
        got_dict, got_error = data_utils.label_class_mean(
            test_df, "one_NA", "label")
    ...
    def test_naive_imputation(self):
        ...
        df = pd.DataFrame(
            {"label": [1, np.nan, 1, 1], "feature_col": [3, 3, np.nan, 3]})
        ...
        df = pd.DataFrame(
            {"label": [1, 1, 1, 1], "feature_col": [3, 3, np.nan, 3]})
        want_df = pd.DataFrame(
            {"label": [1, 1, 1, 1], "feature_col": [3., 3., 3., 3.]})
        got_df, got_error = data_utils.naive_imputation(df, {})
        assert_frame_equal(got_df, want_df)
        self.assertIsNone(got_error)
        ...
        want_df = pd.DataFrame(
            {"label": [1, 1, 1, 1, 2, 2, 2], "feature_col": [3., 3., 3., 3., 7., 7., 7.]})
        got_df, got_error = data_utils.naive_imputation(df, {})
        assert_frame_equal(got_df, want_df)
    ...
    def test_unexpected_na_values_seen(self):
        ...
        self.assertEqual(got_error, "received_featured_but_unreduced_data")
        self.assertTrue(got_na_found)

        # test when the data exists in featured format, but only top-level classification was requested
        got_na_found, got_error = data_utils.unexpected_na_values_seen(
            df, {"generate_features": {}, "reduce_data": {"label_depth": 1}})
        self.assertIsNone(got_error)
        self.assertTrue(got_na_found)

    def test_rowwise_proportion_of_na_smaller_than_boundary(self):
        df = pd.DataFrame({"some_col": [1, 2, 3], "another_col": [
                          2, 3, 4], "col_with_nan": [np.nan, 4, 5]})
        got = [
            x for x in data_utils.rowwise_proportion_of_na_smaller_than_boundary(df, 0.30)]
        want = [False, True, True]
        self.assertListEqual(got, want)

    def test_create_top_level_df(self):
        # test if everything works as intended
        df = pd.DataFrame({"some_col": [1, np.nan], "yet_another_col": [2, np.nan], "another_col": [
                          3, np.nan], "label_top": [1, 1], "label_mid": [2, 2], "index_col": [20, 21]})
        log_file = {"reduce_data": {"needed_columns": [
            ["some_col", "yet_another_col", "another_col"]]}}
        df_top_level_without_subcategories = pd.DataFrame({"some_col": [5, 7], "yet_another_col": [6, 8], "another_col": [
                                                          7, 9], "another_col_2": [np.nan, np.nan], "label_top": [3, 3], "label_mid": [1, 1], "index_col": [30, 31]})
        got = data_utils.create_top_level_df(
            df, log_file, df_top_level_without_subcategories)
        want = pd.DataFrame({"some_col": [1., 5, 7], "yet_another_col": [2., 6, 8], "another_col": [3., 7, 9], "label_top": [
                            1, 3, 3], "label_mid": [2, 1, 1], "index_col": [20, 30, 31], "top_level_marker": [True, True, True]})
        assert_frame_equal(got, want)

    def test_create_mid_level_df(self):
        # test if everything works as intended
        df = pd.DataFrame({"some_col": [1, np.nan, 1], "yet_another_col": [2, np.nan, 2], "another_col": [
                          np.nan, 3, np.nan], "label_top": [1, 1, 1], "label_mid": [2, 2, 1], "index_col": [0, 1, 2]})
        reduced_copy_df = pd.DataFrame({"some_col": [np.nan], "yet_another_col": [
                                       np.nan], "another_col": [3], "label_top": [1], "label_mid": [2], "index_col": [1]})
        log_file = {"reduce_data": {
            "needed_columns": [[], ["another_col"], []]}}
        top_level_df = pd.DataFrame({"index_col": [0, 2]})
        df_top_level_without_subcategories = pd.DataFrame({"some_col": [5]*3, "yet_another_col": [2]*3, "another_col": [
                                                          np.nan]*3, "label_top": [3]*3, "label_mid": [1]*3, "index_col": [x for x in range(3)]})
        want = pd.DataFrame({"another_col": [3.], "label_top": [1], "label_mid": [
                            2], "index_col": [1], "top_level_marker": False})
        got = data_utils.create_mid_level_df(
            df, reduced_copy_df, log_file, top_level_df, df_top_level_without_subcategories)
        assert_frame_equal(got, want)

    def test_split_joined_mid_level_df(self):
        # Test if the label_top col is missing
        got_list, got_error = data_utils.split_joined_mid_level_df(
            pd.DataFrame(), {})
        self.assertEqual(got_error, "label_top_col_not_found")

        # test if everything works as intended
        df = pd.DataFrame({"another_col": [3., np.nan, 7], "yet_another_col": [
                          np.nan, 5, np.nan], "label_top": [1, 2, 1], "label_mid": [2, 1, 3], "index_col": [1, 14, 24]})
        log_file = {"reduce_data": {"needed_columns": [
            [], ["another_col"], ["yet_another_col"]]}}
        want_lifting_df = pd.DataFrame({"another_col": [3., 7], "label_top": [
                                       1, 1], "label_mid": [2, 3], "index_col": [1, 24]})
        want_walking_df = pd.DataFrame({"yet_another_col": [5.], "label_top": [
                                       2], "label_mid": [1], "index_col": [14]})
        got_list, got_error = data_utils.split_joined_mid_level_df(
            df, log_file)
        self.assertIsNone(got_error)
    ...
    def test_create_list_of_dfs_for_imputation(self):
        # Test when the data does not exist in featured format
        got_list, got_error = data_utils.create_list_of_dfs_for_imputation(
            pd.DataFrame(), {})
        self.assertIsNone(got_error)
        ...
        # Test when the data exists in featured format, but was not reduced
        got_list, got_error = data_utils.create_list_of_dfs_for_imputation(
            pd.DataFrame(), {"generate_features": [1]})
        self.assertEqual(got_error, "received_unreduced_data")
        self.assertListEqual(got_list, [])

    def test_detect_label_col(self):
        # Test when the data does not exist in hierarchical featured format
        got_label_name, got_error = data_utils.detect_label_col(
            pd.DataFrame(), {})
        self.assertIsNone(got_error)
        self.assertEqual(got_label_name, "label")

        # Test when the seen columns are a subset of the top_level data
        log_file = {"reduce_data": {"needed_columns": [["some_col"], [
            "another_col"], ["yet_another_col"]]}, "generate_features": True}
        got_label_name, got_error = data_utils.detect_label_col(pd.DataFrame(
            {"some_col": [1, 2], "col_for_internal_processing": [False, True], "label_top": [1, 1]}), log_file)
        self.assertIsNone(got_error)
        self.assertEqual(got_label_name, "label_top")
        ...
        self.assertIsNone(got_error)
        self.assertEqual(got_label_name, "label_mid")
        ...
        got_label_name, got_error = data_utils.detect_label_col(pd.DataFrame({"unseen_col": [
                                                                1, 2], "col_for_internal_processing": [False, True], "label_top": [1, 1]}), log_file)
        self.assertEqual(got_error, "detection_of_label_col_failed")
        self.assertEqual(got_label_name, "")

    def test_create_top_and_mid_df(self):
        # Test if everything works as intended
        df = pd.DataFrame({"some_col": [1, np.nan], "yet_another_col": [2, np.nan], "another_col": [
                          np.nan, 3], "another_col_2": [3, np.nan], "label_top": [1, 1], "label_mid": [2, 2], "label": [12, 12]})
        log_file = {"reduce_data": {"needed_columns": [
            ["some_col", "yet_another_col", "another_col_2"], ["another_col"], []]}, "generate_features": True}
        got_top_level_df, got_mid_level_df, got_error = data_utils.create_top_and_mid_df(
            df, log_file)
        want_top_level_df = pd.DataFrame({"some_col": [1.], "yet_another_col": [2.], "another_col_2": [
                                         3.], "label_top": [1], "label_mid": [2], "index_col": [0], "top_level_marker": [True]})
    ...
    def test_unexpected_na_in_hierarchical_features(self):
        # Test if everything works as intended when no NA values are in the original df
        df = pd.DataFrame({"some_col": [1, np.nan], "yet_another_col": [2, np.nan], "another_col": [
                          np.nan, 3], "another_col_2": [3, np.nan], "label_top": [1, 1], "label_mid": [2, 2], "label": [12, 12]})
        log_file = {"reduce_data": {"needed_columns": [
            ["some_col", "yet_another_col", "another_col_2"], ["another_col"], []]}, "generate_features": True}
        got_unexpected_na_found, got_error = data_utils.unexpected_na_in_hierarchical_features(
            df, log_file)
        self.assertIsNone(got_error)
        self.assertFalse(got_unexpected_na_found)
        ...
        self.assertTrue(got_unexpected_na_found)
        ...
        df = pd.DataFrame({"some_col": [np.nan, np.nan, 1, 2], "yet_another_col": [2, np.nan, 2, 1], "another_col": [
                          np.nan, 3, np.nan, np.nan], "another_col_2": [3, np.nan, 3, 3], "label_top": [1, 1, 1, 1], "label_mid": [2, 2, 2, 2], "label": [12, 12, 12, 12]})
        got_unexpected_na_found, got_error = data_utils.unexpected_na_in_hierarchical_features(
            df, log_file)
        self.assertIsNone(got_error)
        self.assertTrue(got_unexpected_na_found)

    def test_join_list_of_sub_dfs(self):
        # test when one of the elements is not a pd.DataFrame
        got_df, got_error = data_utils.join_list_of_sub_dfs(
            [pd.DataFrame(), 43])
        self.assertEqual(got_error, "got_non_pdDataframe_dt")
        assert_frame_equal(got_df, pd.DataFrame())

        # Test if everything works as intended
        got_df, got_error = data_utils.join_list_of_sub_dfs(
            [pd.DataFrame({"some_col": [1, 2]}), pd.DataFrame({"some_col": [3, 4]})])
        self.assertIsNone(got_error)
    ...
    def test_prepare_labels_hierarchical_classification(self):
    ...
    def test_prepare_label_for_featured_data(self):
    ...
    def test_z_normalize_df(self):
        df = pd.DataFrame({"some_col": [1, 2], "another_col": [10, 20]})
        normalization_df = pd.DataFrame(
            {"some_col": [1.5, 1], "another_col": [15, 10]})
        want = pd.DataFrame(
            {"some_col": [-0.5, 0.5], "another_col": [-0.5, 0.5]})
        got = data_utils.z_normalize_df(df, normalization_df)
        assert_frame_equal(got, want)

    def test_determine_standardization_type(self):
        ...
        self.assertIsNone(got_error)
        self.assertEqual(got_scaling, want_scaling)
    ...
    def test_prepare_standardization_df(self):
        # Test if everything works as intended for z-norm scaling
        folder = "/tmp/trainings_pipeline/evaluation_utils_test/test_prepare_standardization_df"
        if os.path.exists(folder):
            shutil.rmtree(folder)
        os.makedirs(folder)
        standardizing_df = pd.DataFrame(
            {"some_col": [1, 1, 1]}, index=["mean", "scale", "var"])

        standardizing_df.to_csv(os.path.join(
            folder, "test_z_normalize_std.csv"))

        got_standardizing_df, got_standardization_type, got_error = data_utils.prepare_standardization_df(
            folder, True)
        self.assertIsNone(got_error)
        self.assertEqual(got_standardization_type, "normal")
        assert_frame_equal(got_standardizing_df,
                           standardizing_df.reset_index(drop=True))
    ...
    def test_bring_standardizing_df_in_correct_order(self):
        # Test when lifting_data is being handeled
        df = pd.DataFrame({"some_col": [x for x in range(7)]}).T
        want = pd.DataFrame([[0, 4, 1, 2, 3, 5, 6]])
        got_df, got_error = data_utils.bring_standardizing_df_in_correct_order(
            df, "lifting")
        self.assertIsNone(got_error)
        assert_frame_equal(got_df, want)

    def test_prepare_standardization(self):
        # test if everything works as intended
        folder = "/tmp/trainings_pipeline/data_utils_test/test_prepare_standardization"
        if os.path.exists(folder):
            shutil.rmtree(folder)
        os.makedirs(folder)
        standardizing_df = pd.DataFrame(
        ...
        got_df, got_standardization_type, got_error = data_utils.prepare_standardization(
            folder, True, "top")
        self.assertIsNone(got_error)
        self.assertEqual(got_standardization_type, "normal")
