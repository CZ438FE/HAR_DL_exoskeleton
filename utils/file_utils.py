...
def get_files(complete_path: str, file_type="csv"):
    ...
    if all_files == []:
        logger.error(
            f"Given dir {complete_path} does not contain any data of type {file_type} ")
...
def contains_files_with_various_levels(folder_to_check: str, colname: str, max_files_to_check=10):
    ...
    for number, file in enumerate(all_files):
        ...
        if not os.path.exists(file):
            return False, f"file_does_not_exist"

        try:
            df = pd.read_csv(file, usecols=[colname])
        except ValueError:
            logger.error(
                f"Could not find column {colname} in the columns of given file {file}")
...
def debundle_files(path_to_files: str, colname: str):
    ...
    """
    logger.info(
        f"As the given data did contain files with various label levels, debundeling files was started")
    ...
    for file in all_files_to_debundle:
        ...
        logger.debug(f"I created all subfiles for file {file}")

        # And delete the original file with various levels
        os.remove(file)
        logger.debug(f"I deleted unbundled file {file}")
    return None
...
def save_df_with_key(file: str, saving_folder: str, df: pd.DataFrame, key=None):
    ...
    if not isinstance(df, pd.DataFrame):
        logger.error(
            f"Got object df of wrong datatype. Expected pd.DataFrame, got {type(df)}")
...
def save_json(json_formatted_object: dict, full_filename: str):
    ...
    if not os.path.exists(dir_name):
        logger.warning(
            f"The given saving path for the json did not exist. Created it: {dir_name}")
...
def estimate_needed_time(amount_of_files_to_process: int, service_or_method: str, available_cores=os.cpu_count(), size_of_files_to_process_in_GB=0):
    ...
    """
    if service_or_method not in ["create_windows", "generate_features", "balancing_over", "reduce_data", "prepare_dataset", "evaluate_model"]:
        logger.error(
            f"Cannot estimate needed time for service {service_or_method}")
        return "", "invalid_service_or_method_given"

    if service_or_method == "create_windows":
        estimated_dur_min = round(
            amount_of_files_to_process / (available_cores*15))
    if service_or_method == "generate_features":
        estimated_dur_min = round(
            amount_of_files_to_process / (available_cores*70))
    if service_or_method == "balancing_over":
        estimated_dur_min = round(
            amount_of_files_to_process / (available_cores*900))
    if service_or_method == "reduce_data":
        estimated_dur_min = round(
            amount_of_files_to_process / (available_cores*2100))
    if service_or_method == "prepare_dataset":
        estimated_dur_min = round(
            size_of_files_to_process_in_GB * 20 / available_cores)
    if service_or_method == "evaluate_model":
        estimated_dur_min = round(
            size_of_files_to_process_in_GB * 292 / available_cores)
    ...
    # inform the user
    logger.info(
        f"The estimated time to {service_or_method} for these {amount_of_files_to_process} files is {estimated_dur}")
...
def reduce_files_to_handle(list_of_files_to_handle: list, target_location: str, service: str, verbose=True):
    ...
    if "successfully_processed_files" not in log_file[service].keys():
        logger.error(
            f"The given log file does not contain any information regarding successfully_processed_files")
        return [], "log_file_does_not_contain_information_regarding_processed_files"

    if verbose:
        logger.info(
            f"Before removing files which were already processed, {len(list_of_files_to_handle)} files were found do handle")
    ...
    for file in log_file[service]["successfully_processed_files"]:
        try:
            ...
            logger.error(
                f"Already successfully processed file which is not in the list_of_files_to_handle now. Is the same database being processed?")
    ...
    if verbose:
        logger.info(
            f"After removing files which were already processed, {len(list_of_files_to_handle)} are left to handle")
...
def sort_by_filesizes(list_of_files: list, ascending=True):
    ...
    """
    df_filesizes = gather_filesizes_in_df(
        list_of_files).sort_values("size", ascending=ascending)
    return [x for x in df_filesizes["file"]]


def detect_dataset(list_of_files: list):
    """uses the filenames to detect which dataset is currently handeled
    """
    earliest_validation_session = 1664575200
    earliest_test_session = 1673514051

    earliest_observed_unix_time = min(
        [create_unix_time_from_filename(file) for file in list_of_files])
...
def initialize_log_file(future_location: str, prior_location: str, service: str, table: dict, second_prior_location=None, third_prior_location = None):
    ...
    if os.path.exists(prior_location):
        log_file, error = bring_prior_existing_log_file_to_future_location(
            prior_location, service, table, future_location)
        if error:
            return {}, error
        ...
        if second_prior_location is not None:
            ...
            log_file = append_second_location(
                log_file, second_prior_location, service)
        if third_prior_location is not None:
            log_file = append_third_location(
                log_file, third_prior_location, service)

        log_file, error = valid_prior_preprocessing_steps_seen(
...
def create_log_from_result_strings(list_of_result_strings: list, path_to_existing_log_file: str, service: str, separator="+++"):
    """Splits up the result strings to give information regarding the files which needed further investigation and those handeled successfully. saves log if specified
    """
    ...
    if len(log_file[service]["occurred_errors"]) != 0:
        for key in log_file[service]["occurred_errors"].keys():
            logger.warning(
                f"error {key} occurred {len(log_file[service]['occurred_errors'][key])} times")
...
def processing_flattened_data(log_file: dict):
    ...
    if "create_windows" not in log_file.keys():
        logger.error(f"create_windows not found in the keys")
        return False, "create_windows _not_found_in_the_keys"
    if "flatten" not in log_file["create_windows"].keys():
        logger.error(
            f"create_windows does not contain information regarding flattening")
...
def read_csv_safely(file: str, chunksize=1024, header=0):
    """reads in the file chunkwise, therefore causing less strain on the RAM
    """
    df_chunks = pd.read_csv(file, chunksize=chunksize,
                            dtype=np.float64, header=header)
...
def count_filesizes_of_dir(folder: str):
    ...
    if not os.path.exists(folder):
        logger.error(
            f"given folder {folder} does not exist, counting filesizes not possible")
        return 1000.0, "given_dir_does_not_exist"
...
def save_csvs_labelwise(df: pd.DataFrame, folder_to_store_to: str, convolutional=False):
    ...
    list_dicts = [{"df": sub_df.reset_index(drop=True),
                   "filename": os.path.join(folder_to_store_to,  str(sub_df.reset_index(drop=True)["label"][0]) + f"_prepared_{str(uuid.uuid4())}.csv"),
                   "conv":convolutional} for sub_df in list_of_dfs]
    ...
    if occurred_errors != []:
        logger.error(
            f"Encountered these errors while saving df: {occurred_errors}")
...
def remove_outdated_processed_files(log_file: dict, current_processing_step: str):
    ...
    if current_processing_step == "prepare_large_dataset":
        methods_to_check = ["create_windows",
                            "generate_features", "reduce_data", "balancing_over"]
    ...
    for method_to_check in methods_to_check:
        ...
        if method_to_check == "create_windows":
            files_in_input_folder, error = get_files(os.path.join(
                log_file[method_to_check]["data_path"], "labeled_data"), "csv")
            if error:
                return {}, error
        ...
        if method_to_check == "generate_features":
            ...
            if "saving_folder" in log_file["create_windows"].keys():
                ...
                input_folder = os.path.join(
                    log_file["create_windows"]["data_path"], "windows", log_file["create_windows"]["output_date"])
        ...
        if method_to_check == "reduce_data":
            ...
            if "saving_folder" in log_file["generate_features"].keys():
                ...
                input_folder = os.path.join(
                    log_file["generate_features"]["data_path"], "features", log_file["generate_features"]["saving_time"])
        ...
        if method_to_check == "balancing_over":
            ...
            max_saved_files = resulting_files_per_granularity_level(
                log_file, current_processing_step)
...
def update_log_file(log_file: dict, table: dict, saving_folder: str, processing_step: str):
    ...
    log_file, error = remove_outdated_processed_files(
        log_file, processing_step)
    if error:
        return error
...
def get_unique_levels_from_folder(folder: str, col_name: str, data_type, file_type: str):
    ...
    for file in all_files:
        unique_levels = np.append(unique_levels, pd.read_csv(
            file, usecols=[col_name], dtype={col_name: data_type})[col_name].unique())
...
def list_of_maxsized_filechunks(list_of_files: list, maxsize_in_GB: float):
    ...
    non_existing_files = [
        file for file in list_of_files if not os.path.exists(file)]
    if non_existing_files:
        logger.error(
            f"Got {len(non_existing_files)} to process which do not exist")
...
def read_and_check_existing_log_file(file_location: str, table: dict, service: str):
    ...
    for key in table.keys():
        ...
        if log_file[service][key] != table.get(key):
            logger.warning(
                f"Found mismatch between parameters: Current Iteration got parameter {table.get(key)} for {key}, parameter of earlier execution was {log_file[service][key]} ")
...
def valid_prior_preprocessing_steps_seen(service: str, log_file: dict, table: dict):
    ...
    if service == "prepare_dataset":
        if "balancing_over" not in log_file.keys():
            logger.error(
                f"Tried to prepare dataset for training without balancing the data beforehand")
            return {}, "tried_preparing_dataset_without_balancing before"

        if log_file.get("balancing_over").get("granularity") != table.get("granularity"):
            logger.error(
                f"Tried preparing dataset with differing granularity ")
            return {}, f"tried_preparing_dataset_without_differing_granularity_level_chose_{log_file.get('balancing_over').get('granularity')}_for_balancing_received_{table.get('granularity')}"
...
def valid_anonymize_file_found(datapath: str):
    ...
    if not os.path.exists(os.path.join(datapath, "anonymization_file.json")):
        logger.error(
            f"Given Datapath  {datapath} does not contain a anonymization_file.json")
    ...
    if not set(["create_windows", "svm_features", "feature_utils"]).issubset(set(anonymize_file.keys())):
        logger.error(
            f"Not all required keys were found in the anonymization_file.json")
    ...
    if not set(["needed_columns", "data_columns"]).issubset(set(anonymize_file.get("create_windows").keys())):
        logger.error(
            f"Key create_windows does not contain all necessary subkeys")
    ...
    if not set(["top", "lifting", "walking"]).issubset(set(anonymize_file.get("svm_features").keys())):
        logger.error(
            f"Key svm_features does not contain all necessary subkeys")
    ...
    if not set(["accel_cols", "differences_cols", "needed_cols", "on_single_column", "magn_feat_name"]).issubset(set(anonymize_file.get("feature_utils").keys())):
        logger.error(
            f"Key feature_utils does not contain all necessary subkeys")
    ...
    for key in anonymize_file.keys():
        for subkey in anonymize_file.get(key).keys():
            if not isinstance(anonymize_file.get(key).get(subkey), list):
                logger. error(
                    f"Found object in anonymization_file.json which is not a list")
