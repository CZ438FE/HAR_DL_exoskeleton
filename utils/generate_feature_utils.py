...
def valid_table(table: dict):
    ...
    if error:
        logger.error(
            f"Invalid input_date given: {table.get('input_date')} . Please use format YYYY-MM-DDThh:mm. error: {error}")
        return "got_input_date_of_invalid_format"

    # Test if there are valid files in the input folder
    windowed_files, error = file_utils.get_files(os.path.join(
        table.get("data_path"), "windows", table.get('input_date')), "csv")
    if error:
        logger.error(
            f"The folder {os.path.join(table.get('data_path'), 'windows', table.get('input_date'))} does not contain any data in csv form")
        return error
    ...
    log_filepath = os.path.join(
        table.get("data_path"), "windows", table.get('input_date'), "log.json")
    if os.path.exists(log_filepath):
        ...
        if log_file.get("create_windows").get("flatten"):
            logger.error(
                f"The input data was flattened, which is not supported for the generating of features")
            return "generating_features_for_flattened_data_not_allowed"
        if log_file.get("create_windows").get("method") != "ffill":
            logger.error(
                f"Generating of features is only valid with filling method ffill, but {log_file.get('method')} found in the log file of the input data")
...
def generate_features_for_file(file_with_attributes: dict):
    ...
    df_resampled, error_result_string, label = read_prepare_and_ffill_file(
        file, processed_windows)
    ...
    df_boundaries, error = feature_utils.create_df_boundaries(
        df_extended, np.int64(label))
    if error:
        logger.error(
            f"Generation of features not possible without a valid df_boundaries. Error occurred at file {file}")
        return data_utils.create_result_string(file, [1], processed_windows, error)

    # Generate features for each window
    df_features, error = feature_utils.generate_features(
        feature_functions_dict, df_extended, df_boundaries)
    ...
    if store_local:
        return_string, error, processed_windows = save_generated_features(
            df_features, file, file_with_attributes, processed_windows)
...
def save_generated_features(df: pd.DataFrame, file: str, table: dict, processed_windows:int):
    ...
    saving_path = os.path.join(
        table.get("data_path"), "features", table.get("saving_time"), file_name)
    if not os.path.exists(os.path.join(table.get("data_path"), "features", table.get("saving_time"))):
        os.makedirs(os.path.join(table.get("data_path"),
                    "features", table.get("saving_time")))
...
def read_prepare_and_ffill_file(file:str, processed_windows:int):
    """ Returns the resampled and ffilled df, as well as the label, (and the error-result_string, if failed)
    """
    ...
    # Extract the label from the seen df
    ...
    return df_resampled, None, label
