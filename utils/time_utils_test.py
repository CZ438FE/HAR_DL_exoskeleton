...
class TestTimeUtils(unittest.TestCase):
    def test_transform_time_unit(self):
        ...
        precise_df = pd.DataFrame(
            {"numbers": [1666092096000000, 1666092098000000]})
        got_df, got_error = time_utils.transform_time_unit(
            precise_df, "ms", "numbers")
        ...
        got_df, got_error = time_utils.transform_time_unit(
            unprecise_df, "ms", "numbers")
        extrapolated_df = pd.DataFrame(
            {"numbers": [1666000000000, 1666000000000]})
        extrapolated_df["numbers"] = extrapolated_df["numbers"].astype(np.int32)
        assert_frame_equal(got_df, extrapolated_df)
        self.assertIsNone(got_error)

    def test_bring_col_to_greatest_len(self):
        ...
        got_df, got_error = time_utils.bring_col_to_greatest_len(
            test_df, "core")
        ...
        test_df = pd.DataFrame(
            {"numbers": [1, 2, 200], "core": ["12", "2hgjg00", "dsfsdf0"]})
        want_df = pd.DataFrame()
        got_df, got_error = time_utils.bring_col_to_greatest_len(
            test_df, "core")
        assert_frame_equal(got_df, want_df)
        self.assertEqual(got_error, "got_col_core_of_nonint_dt")

        # test if everything works as intended with valid input
        test_df = pd.DataFrame({"numbers": [1, 2, 200], "core": [12, 440, 20]})
        want_df = pd.DataFrame(
            {"numbers": [1, 2, 200], "core": [120, 440, 200]})
        got_df, got_error = time_utils.bring_col_to_greatest_len(
            test_df, "core")
        assert_frame_equal(got_df, want_df)
        self.assertIsNone(got_error)

    def test_time_column_valid(self):
        # Check if it correctly dismisses if the seen date is too small
        test_df = pd.DataFrame({"numbers": [1, 2], "core": [12, 20]})
        self.assertEqual(time_utils.error_in_time_col(
            test_df, "core"), "earliest_date_seen_from_year_smaller_2018")
        # Check if it correctly dismisses if the seen date is too big
        test_df = pd.DataFrame(
            {"numbers": [1, 2], "core": [2041408633505, 2041408633505]})
        self.assertEqual(time_utils.error_in_time_col(
            test_df, "core"), "latest_date_seen_from_year_greater_2023")
        # Check if it correctly dismisses if the seen period is too big
        test_df = pd.DataFrame(
            {"numbers": [1, 2], "core": [1641408633505, 1641408633505 + 10*60*60*1000]})
        self.assertEqual(time_utils.error_in_time_col(
            test_df, "core"), "seen_duration_longer_eight_hours")
        # Check if it correctly accepts if the seen period is valid
        test_df = pd.DataFrame(
            {"numbers": [1, 2], "core": [1641408633505, 1641408633905]})
        self.assertIsNone(time_utils.error_in_time_col(test_df, "core"))

    def test_transform_sci_not(self):
        ...
        test_df = pd.DataFrame(
            {"numbers": [1, 2], "core": ["sdklfhlskdh", "ksdjfnkdfn"]})
        got_df, got_error = time_utils.transform_sci_not(test_df, "core")
        assert_frame_equal(pd.DataFrame(), got_df)
        self.assertEqual("time_col_contains_unknown_datatype", got_error)

        # Test correct processing if the marker E+ is being used, common exponent
        test_df = pd.DataFrame(
            {"numbers": [1, 2], "core": ["1,657007905832E+018", "1,657007905843E+018"]})
        want_df = pd.DataFrame(
            {"numbers": [1, 2], "core": [1657007905832000000, 1657007905843000000]})
        got_df, got_error = time_utils.transform_sci_not(test_df, "core")
    ...
    def test_resample_data(self):
        ...
        test_df = pd.DataFrame(
            {"not_the_time_col": [1667989727000, 1667989727006], "some_col": [12, 14]})
        got_df, got_error = time_utils.resample_data(
            test_df, "not_a_valid_target_unit")
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_error, "time_col_not_found")

        # test when the time col does not contain ints
        test_df = pd.DataFrame(
            {"time": ["1667989727000", "1667989727006"], "some_col": [12, 14]})
        got_df, got_error = time_utils.resample_data(
            test_df, "time_col_has_non_int_entries")
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_error, "time_col_has_non_int_entries")
        ...
        got_df, got_error = time_utils.resample_data(
            test_df, "not_a_valid_target_unit")
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_error, "ms_not_found_in_given_sampling_rate")
        ...
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(
            got_error, "generating_valid_sampling_dur_from_balbms1561_failed")
        ...
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_error, "sampling_rate_smaller_two_given")
        ...
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_error, "sampling_rate_greater_100ms_given")
    ...
    def test_valid_time_string(self):
        ...
        self.assertEqual(got, "got_non_str_time_string")
        ...
        self.assertEqual(got, "wrong_amount_of_-_separators_in_time_string")
        ...
        self.assertEqual(got, "wrong_amount_of_T_separators_in_time_string")

        # test if the separators are correct but the contents are mixed up
        time_string_content_mixed = "01-01-2000T00-00"
        got = time_utils.valid_time_string(time_string_content_mixed)
        self.assertEqual(got, "could_not_gather_time_blocks_from_time_string")
