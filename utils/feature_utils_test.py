...
class TestFeatureUtils(unittest.TestCase):
    def test_expand_df(self):
        ...
        self.assertEqual(got_error, "got_df_without_dtype_pdDataframe")
    ...
    def test_extend_with_acceleration(self):
        ...
        got_df, got_error = feature_utils.extend_with_acceleration(
            df_not_valid, valid_anonymize_file)
        self.assertEqual(got_error, "needed_accel_cols_not_found")
        assert_frame_equal(got_df, pd.DataFrame())

    def test_extend_with_gradients(self):
        df = pd.DataFrame({
            "time": [10, 20, 30, 40, 50],
            "column_a": [1, 0, 0, 4, 0],
            "column_b": [0, 2, 0, 0, 5],
    ...
    def test_is_valid(self):
        ...
        self.assertEqual(feature_utils.is_valid(
            df_wrong_datatype, {}), "got_df_without_dtype_pdDataframe")
        ...
        self.assertEqual(feature_utils.is_valid(df_wrong_colnames, {"feature_utils": {
                         "needed_cols": ["some_col"]}}), "needed_cols_missing")
        ...
        self.assertIsNone(feature_utils.is_valid(valid_df, {"feature_utils": {"needed_cols": [
                          "time", "column_a", "column_b", "column_c", "column_g", "column_h", "column_i", "column_d", "column_e", "column_f"]}}))

    def test_create_df_boundaries(self):
        # Test if the wrong label is given
        valid_df = pd.DataFrame(
            {"labels": [12, 2, 3, 4], "time": [1, 3, 5, 6]})
        wrong_label = 12.2312
        got_df, got_error = feature_utils.create_df_boundaries(
            valid_df, wrong_label)
        self.assertEqual(got_error, "given_label_not_int64")
        assert_frame_equal(got_df, pd.DataFrame())

        # Test if time is not in the columns of the df
        invalid_df = pd.DataFrame({"labels": [12, 2, 3, 4]})
        got_df, got_error = feature_utils.create_df_boundaries(
            invalid_df, np.int64(15))
        ...
        valid_df = pd.DataFrame(
            {"labels": [12, 2, 3, 4], "time": [1, 3, 5, 6]})
        label = np.int64(15)
        got_df, got_error = feature_utils.create_df_boundaries(valid_df, label)
        want = pd.DataFrame(
            {"time_start": [1], "time_end": [6], "label": [label]})
        want = want.astype(
            {"time_start": object, "time_end": object, "label": object})
        self.assertIsNone(got_error)
        assert_frame_equal(got_df, want)

    def test_function_on_window(self):
        df = pd.DataFrame({
        ...
        want = [500]
        got_result, got_error = feature_utils.function_on_window(
            df, df_windows, 'max', 'v_1')
        self.assertCountEqual(got_result, want)
        got_result, got_error = feature_utils.function_on_window(
            df, df_windows, 'max', 'v_1_NaN')
        self.assertCountEqual(got_result, want)

        # test defined function
        want = [2.6227506679548855]
        got_result, got_error = feature_utils.function_on_window(
            df, df_windows, feature_utils.variation_coefficient, 'v_1')
        self.assertCountEqual(got_result, want)
        got_result, got_error = feature_utils.function_on_window(
            df, df_windows, feature_utils.variation_coefficient, 'v_1_NaN')
        self.assertCountEqual(got_result, want)
    ...
    def test_variation_coefficient(self):
        df = pd.DataFrame({
            'v_1': [60, 56, 61, 68, 51, 53, 69, 54, NaN]
        })
    ...
    def test_first_location_of_maximum(self):
        df = pd.DataFrame({
            'v_1': [1, 1, 2, 5, 5, 3, 1, 2, NaN]
        })
        want = [0.375]
        got = feature_utils.first_location_of_maximum(df)
        self.assertCountEqual(got, want)

    def test_power_spectral_entropy(self):
        df = pd.DataFrame({
            'v_1': [-2, 1, NaN, 0, -1, 2],
        })
        want = 0.518
        got = feature_utils.power_spectral_entropy(df)
        self.assertEqual(round(got, 3), want)

    def test_magnitude_area(self):
        df = pd.DataFrame({
            'v_1': [-2, 1, NaN, 0, -1, 2],
        })
        want = 1.0
    ...
    def test_feature_functions_dict(self):
        # test if everything works as intended
        folder = "/tmp/trainings_pipeline/feature_utils_test/test_feature_functions_dict"
        ...
        self.assertTrue(len(got["on_single_column"]) == 4)
        self.assertEqual(got["on_multiple_columns"][0]
