...
class TestGenerateFeatureUtils(unittest.TestCase):
    def test_valid_table(self):
        existing_datapath = "/tmp/trainings_pipeline/generate_feature_utils_test/test_valid_table/"
        ...
        self.assertEqual(generate_feature_utils.valid_table(
        ...
        file_utils.save_json(log_file, os.path.join(
            full_input_folder, "log.json"))
        self.assertEqual(generate_feature_utils.valid_table(
            table), "generating_features_for_flattened_data_not_allowed")
        ...
        file_utils.save_json(log_file, os.path.join(
            full_input_folder, "log.json"))
        self.assertEqual(generate_feature_utils.valid_table(
            table), "generating_features_for_interpolated_data_not_allowed")
        ...
        table = {"data_path": existing_datapath, "dryrun": True,
                 "input_date": valid_date, "output_date": valid_date + valid_date}
        self.assertEqual(generate_feature_utils.valid_table(
            table), "got_time_str_of_invalid_len")

        # test when everything is valid
        table = {"data_path": existing_datapath, "dryrun": True,
                 "input_date": valid_date, "bundle_sessions": True}
        self.assertIsNone(generate_feature_utils.valid_table(table))

    def test_generate_features_for_file(self):
        # test when there are no files in the input folder
        existing_datapath = "/tmp/trainings_pipeline/generate_feature_utils_test/test_generate_features_for_file/"
        if os.path.exists(existing_datapath):
            shutil.rmtree(existing_datapath)

        file_folder = os.path.join(
            existing_datapath, "windows", "2022-01-01T12-00")
        os.makedirs(file_folder)
        full_path_to_file = os.path.join(
            file_folder, "some_file_session_1(1)_1.csv")
        ...
        got = generate_feature_utils.generate_features_for_file(
            file_with_attributes)
        self.assertEqual(got, data_utils.create_result_string(
            full_path_to_file, [1], 0, "file_does_not_exist"))
        ...
        got = generate_feature_utils.generate_features_for_file(
            file_with_attributes)
        self.assertEqual(got, data_utils.create_result_string(
            full_path_to_file, [1], 0, "no_label_col_found"))
        ...
        got = generate_feature_utils.generate_features_for_file(
            file_with_attributes)
        self.assertEqual(got, data_utils.create_result_string(
            full_path_to_file, [1], 0, "received_df_of_too_small_size"))
        ...
        got = generate_feature_utils.generate_features_for_file(
            file_with_attributes)
        self.assertEqual(got, data_utils.create_result_string(
            full_path_to_file, [1], 0, "time_col_not_found"))

        # time_col does not contain ints
        df = pd.DataFrame({"some_col": [1, 2, 3], "label": [11, 11, 11], "time": [
                          "morning", "afternoon", "evening"]})
        df.to_csv(os.path.join(file_folder, "some_file_session_1(1)_1.csv"))
        got = generate_feature_utils.generate_features_for_file(
            file_with_attributes)
        self.assertEqual(got, data_utils.create_result_string(
            full_path_to_file, [1], 0, "time_col_has_non_int_entries"))

        # test when the df does not contain all for expanding the df necessary columns
        df = pd.DataFrame({"some_col": [1, 2, 3], "label": [
                          11, 11, 11], "time": [100, 105, 108]})
        df.to_csv(os.path.join(file_folder, "some_file_session_1(1)_1.csv"))
        got = generate_feature_utils.generate_features_for_file(
            file_with_attributes)
        self.assertEqual(got, data_utils.create_result_string(
            full_path_to_file, [1], 0, "needed_cols_missing"))
        ...
        want = data_utils.create_result_string(
            full_path_to_file, [None], 1, "None")
        if "win" in sys.platform:
            want = want.replace("\\","/")
        self.assertEqual(got, want)

        # Test if the file exists
        resulting_filename = os.path.join(
            file_folder, "some_file_session_1(1)_1.csv").replace("windows", "features")
        exists = os.path.exists(resulting_filename)
        self.assertTrue(exists)

        # Check if the df has the correct dimensionality
        df_read_in = pd.read_csv(resulting_filename)
        self.assertTupleEqual(df_read_in.shape, (1, 47))

    def test_save_generated_features(self):
        ...
        got_result_string, got_error, got_handeled_windows = generate_feature_utils.save_generated_features(
            valid_df, "somefile(1)_1.csv", malformed_table, 52)
        self.assertEqual(got_error, "not_all_needed_keys_in_table_for_saving")
        self.assertEqual(got_handeled_windows, 52)
        want = data_utils.create_result_string(
            "somefile(1)_1.csv", [1], 52, "not_all_needed_keys_in_table_for_saving")
        self.assertEqual(got_result_string, want)

    def test_read_prepare_and_ffill_file(self):
        # test if the given file does not exist
        existing_datapath = "/tmp/trainings_pipeline/generate_feature_utils_test/test_rear_prepare_and_ffill_df/"
        if os.path.exists(existing_datapath):
            shutil.rmtree(existing_datapath)
        os.makedirs(existing_datapath)
        non_existing_file = os.path.join(existing_datapath, "non_existing.csv")
        got_df, got_result_string, got_label = generate_feature_utils.read_prepare_and_ffill_file(
            non_existing_file, 52)
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_result_string, data_utils.create_result_string(
            non_existing_file, [1], 52, "file_does_not_exist"))
        self.assertEqual(got_label, 0)
        ...
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_result_string, data_utils.create_result_string(
            path_to_file, [1], 52, "no_label_col_found"))
        self.assertEqual(got_label, 0)
        ...
        assert_frame_equal(got_df, pd.DataFrame())
        self.assertEqual(got_result_string, data_utils.create_result_string(
            path_to_file, [1], 52, "received_df_of_too_small_size"))
        self.assertEqual(got_label, 0)
        ...
        got_df, got_result_string, got_label = generate_feature_utils.read_prepare_and_ffill_file(
            path_to_file, 52)
        self.assertIsNone(got_result_string)
        self.assertEqual(got_label, 1)
        assert_frame_equal(got_df, pd.DataFrame({"time": [102]}))

    def test_create_list_of_file_dicts(self):
        existing_datapath = "/tmp/trainings_pipeline/generate_feature_utils_test/test_create_list_of_file_dicts/"
        if os.path.exists(existing_datapath):
            shutil.rmtree(existing_datapath)
        os.makedirs(existing_datapath)
        ...
        got_list, got_error = generate_feature_utils.create_list_of_file_dicts(
            all_window_files, table, {}, valid_anonymize_file)
        self.assertIsNone(got_error)
        self.assertListEqual(want, got_list)
