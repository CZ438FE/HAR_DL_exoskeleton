...
def add_args(group):
    ...
    group.add_argument(
        ...
        required=True,
        type=str,
        help='Specify the date to identify for which windowed data to generate features for'
    )
    group.add_argument(
        '-o', '--output_date',
        metavar='',
...
def generate_features(table: dict):
   ...
   # For better readability the variables are created internally from the table
    ...
    # When the results are being stored and the respective folders do not exist yet, create them at <dp>/features/<saving_time>
    saving_time = datetime.now().strftime(
        '%Y-%m-%dT%H-%M') if output_date is None else output_date
    table["saving_time"] = saving_time
    saving_folder = os.path.join(data_path, "features", saving_time)
    table["saving_folder"] = saving_folder
    if store_local and not os.path.exists(saving_folder):
        os.makedirs(saving_folder)

    all_window_files, error = file_utils.get_files(
        os.path.join(data_path, "windows", input_date), ".csv")
    if error:
        return
    ...
    log_file, error = file_utils.initialize_log_file(os.path.join(saving_folder, "log.json"), os.path.join(
        data_path, "windows", input_date, "log.json"), service, table)
    if error:
        logger.error(f"Initializing log file failed: {error}")
        return

    all_window_files, error = file_utils.reduce_files_to_handle(
        all_window_files, saving_folder, service)
    ...
    feature_functions_dict, error = generate_feature_utils.feature_utils.feature_functions_dict(
        data_path)
    if error:
        return
    ...
    list_of_file_dicts, error = generate_feature_utils.create_list_of_file_dicts(
        all_window_files, table, feature_functions_dict, anonymize_file)
    if error:
        return

    # Analogue to the creation of windows, multiprocessing is used, which is being done chunkwise
    session_chunks = data_utils.split_files_to_handle_into_chunks(
        list_of_file_dicts, 96)

    logger.info(
        f"Finished first processing steps, Starting with generating of features")

    for chunk_number, chunk in enumerate(session_chunks, start=1):
        # Process the files in the chunk with all available CPU cores
        with Pool() as pool:
            result = pool.map(
                generate_feature_utils.generate_features_for_file, chunk)
    ...
    logger.info(
        f"Finished creating windows. Duration: {duration} min for {len(all_window_files)} files")
