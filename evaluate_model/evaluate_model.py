...
def add_args(group):
    ...
    group.add_argument(
        ...
        required=True,
        type=str,
        help='Specify the folder in which to search for the model'
    )
    group.add_argument(
        '-test_d', '--test_data',
        metavar='',
        ...
        required=True,
        type=str,
        help='Specify the folder in which to search for the test data'
    )
    group.add_argument(
        '-o', '--output_date',
        metavar='',
...
def evaluate_model(table: dict):
    ...
    log_file, error = file_utils.initialize_log_file(os.path.join(saving_folder, "log.json"), os.path.join(
        model_folder, "log.json"), service, table, os.path.join(test_data, "log.json"))
    if error:
        logger.error(f"Initializing log file failed: {error}")
        return

    error = evaluation_utils.valid_preprocessing_of_data_for_model(
    ...
    confusion_matrix, accuracy, predictions_df, prediction_time, error = evaluation_utils.create_predictions(
        model, test_data, model_folder, store_local, saving_folder, False, 1.0)
    if error:
        return
