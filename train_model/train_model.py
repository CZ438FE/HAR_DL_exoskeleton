...
def check_table_args(table):
    ...
    if "epochs" not in table or table["epochs"] is None:
        table["epochs"] = 1
    if "loss_function" not in table:
        table["loss_function"] = "CEL"
    if "optimizer" not in table:
        table["optimizer"] = "adam"
    if "layer_structure" not in table:
        table["layer_structure"] = "250f150f50"
    if "dropout_rate" not in table:
        table["dropout_rate"] = None
    if "batch_normalization" not in table:
        table["batch_normalization"] = False
    if "learning_rate" not in table:
        table["learning_rate"] = 0.001
...
def train_model(table: dict):
    ...
    # When the results are being stored and the respective folders do not exist yet, create them at <dp>/train_model/<saving_time>
    saving_time = datetime.now().strftime(
        '%Y-%m-%dT%H-%M') if output_date is None else output_date
    saving_folder = os.path.join(
        data_path, "train_model", network_type,  saving_time)
    ...
    log_file, error = file_utils.initialize_log_file(os.path.join(saving_folder, "log.json"), os.path.join(
        training_data, "log.json"), service, table, validation_data_log_pos)
    if error:
    ...
    if network_type not in implemented_network_types:
        logger.error(
            f"Got invalid network type, please choose from the list of implemented ones in the README")
        return

    trainloader, error = train_model_utils.create_loader(
        training_data,  minibatch_size, "training", epochs, hierarchical_model)
    if error:
        return

    # The nr_of_features and the number of target classes are needed for the construction of the Network
    n_features, out_szs, error = train_model_utils.get_n_features_and_out_sz(
        training_data, hierarchical_model)
    if error:
        return
    ...
    if validation_data:
        validationloader, error = train_model_utils.create_loader(
            validation_data, minibatch_size, "validation", 1, hierarchical_model)
        if error:
            return

        # Test if the dimensionality of the validation data is equal to that of the trainings-data
        n_features_validation, out_szs_validation, error = train_model_utils.get_n_features_and_out_sz(
            validation_data, hierarchical_model)
        if error:
            return
        if n_features != n_features_validation:
            logger.error(
                f"Amount of features seen in validation ({n_features_validation}) and train data ({n_features}) differs")
            return
        if out_szs != out_szs_validation:
            logger.error(
                f"Amount of target classes seen in validation ({out_szs_validation}) and train data ({out_szs}) differs")
            return
    ...
    Model = train_model_utils.build_nn_model(
        network_type, batch_normalization, seed, log_file)
    ...
    log_file[service]["nr_model_params"] = sum(
    ...
    optim, error = train_model_utils.set_optimizer(
        optimizer, learning_rate, model)
    if error:
        return
    ...
    if number_minibatches:
        nr_minibatches_to_train = min(
            [nr_minibatches_to_train, number_minibatches])
    ...
    for current_epoch in range(math.ceil(remaining_epochs_to_train)):
        ...
        for nr_batch, sample_batched in enumerate(trainloader):
            ...
            y_truth = train_model_utils.bring_to_correct_format(
                sample_batched[1])
            ...
            losses.append(loss) if isinstance(
                loss, float) else losses.append(loss.item())
            ...
            nr_correct_classifications = train_model_utils.sum_identical_entries(
                y_truth, y_pred_transformed)
            train_accuracies.append(
                nr_correct_classifications.item()/len(y_pred_transformed))

            if nr_batch % 160 == 0 or nr_batch == 0:
                if nr_batch == 160 and current_epoch == 0:
                    logger.info(
                        f"Training for {int(nr_minibatches_to_train)} minibatches will take {int((time.time()-start_time)*nr_minibatches_to_train/12000)} minutes")
                logger.info(
                    f'epoch: {table["already_trained_epochs"]+1} minibatch {nr_batch} Trainings-data accuracy: {round(train_accuracies[-1]*100,2)}%')

            if validation_data and nr_batch % amount_of_minibatches_to_train_before_validating == 0:
                validation_accuracies, validation_losses, error = train_model_utils.evaluate_on_validation_data(
                    model, saving_folder, validation_losses, validation_accuracies, validationloader, criterion, 1, True, "Minibatch", nr_trained_minibatches, network_type, log_file)
                if error:
                    return
        ...
        if validation_data:
            validation_accuracies, validation_losses, error = train_model_utils.evaluate_on_validation_data(
                model, saving_folder, validation_losses, validation_accuracies, validationloader, criterion, 1, False, "Epoch", current_epoch+1, network_type, log_file)
            if error:
                return

    logger.info(
        f"Finished training. Duration: {int((time.time() - start_time)/60)} min")

    # If validation_data was given, estimate the accuracy and a confusion matrix for the resulting model on the validation data
    if validation_data:
        _, final_validation_accuracy, _ = train_model_utils.display_confusion_matrix(
            validationloader, model, "validation", store_local, saving_folder, False, 1.0, hierarchical_model)
        log_file[service]["final_validation_accuracy"] = final_validation_accuracy
        log_file[service]["max_validation_accuracy"] = max(
            validation_accuracies)

        minibatch_nr_max_validation_acc = (validation_accuracies.index(
            max(validation_accuracies)))*amount_of_minibatches_to_train_before_validating
        logger.info(
            f"The best validation accuracy was archived at minibatch nr [{minibatch_nr_max_validation_acc}/{len(validation_accuracies)*amount_of_minibatches_to_train_before_validating}] with {round(max(validation_accuracies)*100, 2)} % ")
        log_file[service]["minibatch_nr_max_validation_acc"] = minibatch_nr_max_validation_acc
        log_file[service]["nr_minibatches_trained"] = math.ceil(
            nr_minibatches_to_train)

    # Create a classification matrix for the trainings-data
    _, final_training_accuracy, _ = train_model_utils.display_confusion_matrix(
        trainloader, model, "training", store_local, saving_folder, False, 0.25,  hierarchical_model)
    ...
    log_file[service]["training_date"] = datetime.now().strftime(
        '%Y-%m-%dT%H-%M')

    # Visualize the results
    if plot:
        plot_utils.plot_accuracies_and_loss_over_batches(
            train_accuracies, validation_accuracies, losses, validation_losses, store_local, saving_folder, amount_of_minibatches_to_train_before_validating)
